...
| **Q3** | 公共 KV 块多 Reader Attention | 主机端共享 World 已交付；生产 grouped-head 路径（GQA 组内 KV tile 共 staging）已在 `flash_attn_rerot.comp` 落地，`GGML_VK_REROT_GROUPED_HEADS=1` opt-in | 2026-09-25 配对模型资格赛：每 episode attention 18.7s→4.2s，精度门 <1.2e-7，test-rerot-attn 0 failure；但同 seed episode 确定性泄漏 become 工具调用为公开答案（标量路径同 seed 干净收尾），配对运行失败 | 多 Reader 间的 KV 重复加载已由 L2 吸收（n=3 dispatch 边际成本仅 +106µs/reader，剩余为不可共享的 QK^T+softmax）；瓶颈不在供给冗余而在索引着色器本身效率（273 vs 725 GFLOPS） | **保持 opt-in**（资格赛失败，Br=1 默认不变） | 协议层先修最终 fence 拒绝内部工具调用泄漏（与 attention 路径无关的开放缺陷）；修复后重跑配对资格赛，通过则默认启用 grouped 路径。 |
| **Q5** | GDN 状态共同基底 + 低秩增量 | CPU 数学参考层已交付（`llama-rerot-math.*`）；生产路径无任何低秩表示 | F32 数值门实测通过（相对误差 ~1.9e-7 有界 / ~5.1e-7 弱衰减） | 2026-09-25 算术否决：lane 逐 token 分岐使 $B^T k_i$ 无跨 lane 共享；FLOP 交叉点在 $r=16$，重物化阈值 $r^*\approx32$，每 32 步重物化使每步成本比稠密高 40%（114.7k vs 81.9k FLOPs/head）；且稠密 128×128 GEMV 恰好映射 SIMD wavefront，低秩形式反而引入变长循环与寄存器溢出 | **否决**（GPU 负收益，非未实现） | 不推进；若未来 lane 在同一步共享同一 token（如 MTP verify 网格），重算交叉点后重开。 |
