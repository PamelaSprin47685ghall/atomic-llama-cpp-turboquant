...
| **Q3** | 公共 KV 块多 Reader Attention | 主机端共享 World 已交付；生产 grouped-head 路径（GQA 组内 KV tile 共 staging）已落地且**自 2026-09-26 起为默认**（`GGML_VK_REROT_GROUPED_HEADS=0` 回退 Br=1） | 2026-09-26 资格赛通过：每 episode attention 18.7s→4.2s（273→478 GFLOPS/s），精度门 <1.2e-7，test-rerot-attn 0 failure；三 seed 背靠背配对运行 3/3 干净收尾（两处前置缺陷——become 逐字符泄漏与 force-seal slot 残留——均已在 grouped 之外修复） | 多 Reader 间 KV 重复加载已由 L2 吸收（n=3 边际 +106µs/reader）；grouped 路径把索引着色器效率从 2.7x 差距收窄 | **已生产化（默认开启）** | 后续：在更长上下文（>16K）与多 episode 并发下复测 grouped 收益曲线。 |
