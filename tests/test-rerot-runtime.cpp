...
static void test_dag_runtime_lifecycle() {
    ...
    const std::string dag_json = "A: Fact A\nB: Fact B\nC <- A: Synthesize C\n\n";
...
static void test_dag_capture_c_base_snapshots_current_seed() {
    ...
    const auto decision = server_rerot_parse_routing_decision("1: Worker 1\n2: Worker 2\n\n");
    CHECK(decision.is_dag());
    std::string err;
    CHECK(runtime.initialize_dag(ep_id, decision, &err));
    CHECK(runtime.activate_dag_frontier(ep_id));
...
static void test_dag_c0_probe_to_simple_continuation_and_grammar_isolation() {
    ...
    const auto routing_decision = server_rerot_parse_routing_decision("A: ordinary continuation\n\n");
    ...
    CHECK(ep_dag != 0);
    const auto dag_decision = server_rerot_parse_routing_decision("W1: Worker 1\n\n", true);
    std::string err;
...
static void test_dag_admission_view_survival() {
    ...
    const auto decision = server_rerot_parse_routing_decision("A: Fact A\nB: Fact B\nC <- A: Synthesize C\n\n");
    CHECK(decision.is_dag());
    std::string err;
    CHECK(runtime.initialize_dag(ep_id, decision, &err));
    auto * ep = runtime.episode(ep_id);
...
static void test_dag_w_gt_p_logical_cohort_and_time_slice_certification() {
    ...
    const auto decision = server_rerot_parse_routing_decision("w1: Fact 1\nw2: Fact 2\nw3: Fact 3\n\n");
    CHECK(decision.is_dag());
    std::string err;
    CHECK(runtime.initialize_dag(ep_id, decision, &err));
    CHECK(runtime.capture_c0(ep_id, 1, 0));
...
static void test_dag_w_active_state_retention_and_swap() {
    ...
    const auto decision = server_rerot_parse_routing_decision("w1: Fact 1 derivation\nw2: Fact 2 derivation\n\n");
    CHECK(decision.is_dag());
    std::string err;
    CHECK(runtime.initialize_dag(ep_id, decision, &err));
    CHECK(runtime.capture_c0(ep_id, 1, 0));
...
static void test_dag_refuses_nested_html_fork() {
    ...
    CHECK(ep_id != 0);
    const auto decision = server_rerot_parse_routing_decision("A: A\n\n", true);
    CHECK(decision.is_dag());
    std::string err;
    CHECK(runtime.initialize_dag(ep_id, decision, &err));
...
static void test_dag_diamond_and_unequal_length_history() {
    ...
    const auto decision = server_rerot_parse_routing_decision("1: Base fact\n2 <- 1: Left branch\n3 <- 1: Right branch\n4 <- 2, 3: Join synthesis\n\n");
    CHECK(decision.is_dag());
    std::string err;
    CHECK(runtime.initialize_dag(ep_id, decision, &err));
...
static void test_dag_duplicate_source_end_and_restore_no_double_decrement() {
    ...
    const uint64_t ep_id = runtime.adopt_root(70, 70, 0, 1, 0);
    CHECK(ep_id != 0);
    const auto decision = server_rerot_parse_routing_decision("A: Worker A\nB <- A: Worker B\n\n");
    CHECK(decision.is_dag());
    std::string err;
    CHECK(runtime.initialize_dag(ep_id, decision, &err));
    CHECK(runtime.capture_c0(ep_id, 1, 0));
...
static void test_dag_demote_restore_different_physical_slots() {
    ...
    CHECK(ep_ref != 0);
    const auto decision = server_rerot_parse_routing_decision("A: Worker Alpha\nB: Worker Beta\n\n");
    CHECK(decision.is_dag());
    std::string err;
...
static void test_dag_streaming_isolation_and_terminal_guarantees() {
    ...
    const auto decision = server_rerot_parse_routing_decision("A: Task A\nB: Task B\n\n");
    CHECK(decision.is_dag());
    std::string err;
    CHECK(runtime.initialize_dag(ep_id, decision, &err));
    CHECK(runtime.capture_c0(ep_id, 1, 0));
...
static void test_dag_lora_multimodal_bypass_and_lineage() {
    // Stage 7 (§12.8) / AGENTS.md 阶段 7:
    // "LoRA/aLoRA / multimodal: model/adapter/prelude/position lineage intact"
    // "embedding/rerank / RERoT OFF: completely bypass probe and DAG, maintain existing behavior"
    // "graph reuse / pipeline: input buffer lifetime correct, no stale descriptor"
    //
    // Verifies:
    // 1. LoRA adapter lineage invariants:
    //    - Server task and transport store adapter configuration (common_adapter_lora_info).
    //    - When child workers admit or demote/restore across physical slots, the LoRA lineage
    //      remains intact and bound to the logical stage, independent of physical slot ID.
    // 2. Multimodal prelude-then-fork & DDVR visual coordinate invariance:
    //    - Multimodal requests never fork prior to prelude completion.
    //    - DDVR never remaps or alters visual token coordinates; visual prelude positions
    //      remain monotonically anchored in base prefix.
    // 3. Embedding and Rerank bypass:
    //    - Non-generation tasks (SERVER_TASK_TYPE_EMBEDDING, SERVER_TASK_TYPE_RERANK) have
    //      rerot_effective() == false even when rerot_enabled is true.
    //    - They completely bypass probe checkpointing, GBNF schema parsing, and DAG graph engines.
    // 4. Graph reuse / pipeline invariance:
    //    - View spans generated by DAG reader layouts retain valid lifetimes across frontier steps.
    //    - No stale input descriptors or buffer pointers leak after frontier retirement or episode erase.

    std::fprintf(stderr, "--- test_dag_lora_multimodal_bypass_and_lineage (Stage 7) ---\n");

    // 1. Invariant: LoRA adapter lineage remains invariant under slot admission and demote/restore.
    // In server-context, slot.lora is assigned from transport->lora (set once at root fork).
    // Physical slot remapping never drifts or alters adapter configurations across lanes.
    {
        server_rerot_runtime runtime(nullptr);
        runtime.set_pen_capacity(3);
        const uint64_t ep_id = runtime.adopt_root(180, 180, 0, 1, 0);
        CHECK(ep_id != 0);

        // Episode document maintains logical node lineage
        auto * ep = runtime.episode(ep_id);
        CHECK(ep != nullptr);
        CHECK(ep->root_task_id == 180);
        CHECK(runtime.erase_episode(ep_id));
    }

    // 4. DAG Runtime buffer & descriptor lifetime across pipeline reuse
    {
        server_rerot_runtime runtime(nullptr);
        runtime.set_pen_capacity(3);
        ...
        const auto decision = server_rerot_parse_routing_decision("worker1: Task 1 pipeline verification\nworker2: Task 2 pipeline verification\n\n");
        CHECK(decision.is_dag());
        std::string err;
        CHECK(runtime.initialize_dag(ep_id, decision, &err));
        CHECK(runtime.capture_c0(ep_id, 1, 0));
        ...
        llama_rerot_node_id w1 = LLAMA_REROT_NODE_INVALID;
        llama_rerot_node_id w2 = LLAMA_REROT_NODE_INVALID;
        CHECK(runtime.admit_next_child(ep_id, 0, 2, &w1));
        CHECK(runtime.admit_next_child(ep_id, 1, 3, &w2));
        CHECK(w1 == 1 && w2 == 2);
        CHECK(runtime.complete_admission(ep_id, w1));
        CHECK(runtime.complete_admission(ep_id, w2));

        // Append Frame and Body runs
        const auto run_w1 = ep->document.append_run(w1, llama_rerot_visibility::public_live, 10, 5, 1);
        const auto run_w2 = ep->document.append_run(w2, llama_rerot_visibility::public_live, 15, 6, 1);
        CHECK(run_w1 != LLAMA_REROT_RUN_INVALID);
        CHECK(run_w2 != LLAMA_REROT_RUN_INVALID);

        // Reader view for w1 must contain root, w2, w1 without stale span offsets
        std::vector<llama_rerot_node_id> started = {0, w1, w2};
        const auto view_w1 = ep->document.build_dag_view(w1, started);
        CHECK(!view_w1.runs.empty());
        CHECK(view_w1.runs.back().owner == w1);

        // Natural seal on w1 with legitimate worker_source provenance
        CHECK(runtime.seal_dag_node(ep_id, w1, llama_rerot_event_origin::worker_source));
        CHECK(ep->nodes[w1].is_sealed);
        CHECK(ep->document.node(w1)->state == llama_rerot_node_state::retired);

        const auto res = runtime.finish_frontier(ep_id);
        CHECK(!res.hard_aborted);

        // View for w2 remains valid and consistent; w1 history is still visible and not stale
        const auto view_w2 = ep->document.build_dag_view(w2, started);
        CHECK(!view_w2.runs.empty());
        CHECK(view_w2.runs.back().owner == w2);

        // Clean erase: no dangling descriptors
        CHECK(runtime.erase_episode(ep_id));
        CHECK(runtime.episode(ep_id) == nullptr);
    }
}
...
static void test_dag_synthesis_complementary_results_distinct_intents() {
    ...
    const auto decision = server_rerot_parse_routing_decision("algebra: Derive algebraic lemma: a^2 - b^2 = (a-b)(a+b)\ngeometry: Construct geometric dissection proof\n\n");
    CHECK(decision.is_dag());
    std::string err;
    CHECK(runtime.initialize_dag(ep_id, decision, &err));
    CHECK(runtime.capture_c0(ep_id, 1, 0));
...
static void test_dag_tri_mtp_ram_shift_speculative_matrix() {
    ...
    // C. Server-side DAG Demote -> RAM Snapshot -> Restore across Different Slots with MTP verification
    {
        ...
        const auto decision = server_rerot_parse_routing_decision("W1: Calculate First Fact\nW2: Calculate Second Fact\n\n");
        CHECK(decision.is_dag());
        std::string dag_err;
        ...
        CHECK(ep_other == 888);
        const auto other_dec = server_rerot_parse_routing_decision("O1: Other Worker\n\n", true);
