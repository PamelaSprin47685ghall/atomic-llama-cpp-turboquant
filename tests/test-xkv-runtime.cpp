...
// ---------------------------------------------------------------------------
// ---------------------------------------------------------------------------
// 18b. Shared-reader multi-query rows through the REAL cache builder: the
// MTP-verify shape (one pen, several query rows in one ubatch, one reader
// view). rerot_build_attn_layout groups rows by reader state and runs the
// structure pass once; this regression pins the layout contract (per-query
// contiguous entry runs, group ownership, causal cuts per row, own-row
// lookup, effective-position monotonicity) against the shared path.
// ---------------------------------------------------------------------------
static void test_rerot_shared_reader_multi_query() {
    fprintf(stderr, "--- test_rerot_shared_reader_multi_query ---\n");
    rt_test_model model(2);
    llama_cparams cp = rt_cparams(LLAMA_XKV_MODE_SHADOW, 8, 16, 2);
    cp.rerot_enabled = true;
    llama_kv_cache kv(model, model.hparams, GGML_TYPE_F32, GGML_TYPE_F32,
    ...
    // Root public run 1 (foreign, older frontier): FULL arm for the reader.
    llama_kv_rerot_meta root_tag;
    root_tag.episode_id = 7; root_tag.node_id = 1; root_tag.run_id = 1;
    root_tag.publish_epoch = 1; root_tag.frontier = 3;
    root_tag.visibility = llama_rerot_visibility::public_live;
    auto b_root = commit_tokens(kv, 0, {0, 1, 2}, true, &root_tag);
    write_layer_rows(kv, 0, b_root, {0, 1, 2});

    // Own public run 2 (current frontier): causally gated arm.
    llama_kv_rerot_meta own_tag;
    own_tag.episode_id = 7; own_tag.node_id = 2; own_tag.run_id = 2;
    own_tag.publish_epoch = 0; own_tag.frontier = 4;
    own_tag.visibility = llama_rerot_visibility::public_live;
    auto b_own = commit_tokens(kv, 0, {4, 5, 6}, true, &own_tag);
    write_layer_rows(kv, 0, b_own, {4, 5, 6});

    // Own private run 3: gated arm, own-node only.
    llama_kv_rerot_meta priv_tag;
    priv_tag.episode_id = 7; priv_tag.node_id = 2; priv_tag.run_id = 3;
    priv_tag.publish_epoch = 0; priv_tag.frontier = 0;
    priv_tag.visibility = llama_rerot_visibility::private_control;
    auto b_priv = commit_tokens(kv, 0, {8, 9}, true, &priv_tag);
    write_layer_rows(kv, 0, b_priv, {8, 9});

    llama_rerot_reader_state reader;
    reader.episode_id = 7; reader.reader = 2; reader.query_run = 2;
    reader.frontier = 4; reader.frontier_mode = LLAMA_REROT_FRONTIER_STRONG;
    reader.ordered_runs = {1, 3, 2};
    TEST_ASSERT(kv.rerot_set_reader_view(0, reader));

    // MTP-verify shape: 5 rows of ONE sequence (one reader view), distinct
    // positions spanning before/inside/after every run boundary.
    const std::vector<llama_pos> positions = {5, 9, 4, 6, 0};
    std::vector<llama_token> tokens(positions.size(), 101);
    std::vector<int32_t> n_seq_id(positions.size(), 1);
    llama_seq_id seq0 = 0;
    ...
    TEST_ASSERT(layout.n_queries == positions.size());
    TEST_ASSERT(layout.query_offsets.size() == positions.size() + 1);
    ...
    // shared path against the per-query contract, cache-level.
    ...
    for (uint32_t idx = 0; idx < cells0.size(); ++idx) {
        ...
        oracle_keys.push_back({ idx, cells0.pos_get(idx), cells0.seq_has(idx, seq0), cells0.rerot_get(idx) });
    }
    for (uint32_t q = 0; q < positions.size(); ++q) {
        const auto oracle = llama_rerot_build_query_layout(reader, positions[q], oracle_keys);

        // Per-query entry run is contiguous and sized like the oracle's.
        const uint32_t e0 = layout.query_offsets[q];
        const uint32_t e1 = layout.query_offsets[q + 1];
        TEST_ASSERT(e1 - e0 == oracle.entries.size());
        // Same (key, effective) multiset — the layout contract the backend
        // consumes (entries reference groups; groups carry effective_pos).
        ...
        for (uint32_t e = e0; e < e1; ++e) {
            ...
            TEST_ASSERT(layout.groups[entry.group_index].query_index == q);
            TEST_ASSERT(entry.key_index < cells0.size());
            got.emplace_back(entry.key_index, layout.groups[entry.group_index].effective_pos);
        }
        ...
        // Causal cut per row: no entry's key storage exceeds the row's
        // query position for own-node rows (foreign FULL rows are exempt).
        for (uint32_t e = e0; e < e1; ++e) {
            const auto & meta = cells0.rerot_get(layout.entries[e].key_index);
            if (meta.node_id != reader.reader) continue;
