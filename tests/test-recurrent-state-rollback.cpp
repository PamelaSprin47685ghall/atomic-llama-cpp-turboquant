...
// contexts must be released on every exit path, including the failure ones
using ctx_ptr = std::unique_ptr<llama_context, void (*)(llama_context *)>;

static ctx_ptr wrap_ctx(llama_context * ctx) {
    return ctx_ptr(ctx, llama_free);
}
...
int main(int argc, char ** argv) {
    ...
    ctx_ptr ctx_src = wrap_ctx(make_ctx(params, model));
    ctx_ptr ctx_dst = wrap_ctx(make_ctx(params, model));
    if (!ctx_src || !ctx_dst) {
    ...
    // Replay the rolled-back tail on the source and on a restored target,
    // comparing logits position by position.
    //
    // The baseline is re-read from ctx_src inside every round on purpose. Each
    // replay rewrites the source's snapshot ring, so a checkpoint taken before
    // one round no longer describes the source's state during a later round.
    // Holding a baseline across rounds compares two different states and
    // reports a phantom restore failure.
    const auto replay_and_compare = [&](const char * mode, llama_context * ctx_tgt) {
        for (uint32_t i = 0; i < n_rollback; ++i) {
            const llama_pos pos = rollback_pos + i;
            if (!decode_one(ctx_src.get(), tokens[pos], pos) ||
                !decode_one(ctx_tgt, tokens[pos], pos)) {
            ...
            if (logits_src == nullptr || logits_tgt == nullptr) {
    ...
    if (!replay_and_compare("full", ctx_dst.get())) {
        return 1;
    }
    ...
    ctx_ptr ctx_dirty = wrap_ctx(make_ctx(params, model));
    if (!ctx_dirty) {
    ...
    // Roll the source back once more and re-take the checkpoint, so the bytes
    // loaded into the dirty context and the baseline logits that ctx_src
    // produces below describe one and the same source state.
    if (!llama_memory_seq_rm(llama_get_memory(ctx_src.get()), 0, rollback_pos, -1)) {
        fprintf(stderr, "%s : dirty-round source rollback failed\n", __func__);
        return 1;
    }
    ckpt.update_tgt(ctx_src.get(), 0, 0);
    ckpt.load_tgt(ctx_dirty.get(), 0, 0);

    if (!replay_and_compare("dirty-ctx", ctx_dirty.get())) {
        return 1;
    }
