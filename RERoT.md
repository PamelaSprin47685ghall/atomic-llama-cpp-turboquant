...
**grouped-head attention 资格赛通过，成为默认。** `GGML_VK_REROT_GROUPED_HEADS`（每 subgroup 一个 Q head，GQA 组内 KV tile 只 staging 一次）首次跑完整配对 episode：每 episode attention 成本从 18.7 s 降到 4.2 s（单次 dispatch 460 µs→197 µs，n_kv=4096，273→478 GFLOPS/s），索引路径对普通 FA 的效率差距从 2.7x 显著收窄；精度门两侧均过（indexed vs F64 < 1.2e-7；test-rerot-attn 全量 0 failure）。首次配对运行曾失败（同 seed 确定性泄漏 `<|fim_prefix|>become` 工具调用为公开答案，清单 0/9），但两个缺陷均定位、修复并复验，且都不在 grouped 着色器内：① synthesis 逐字符拼写保留标记绕过 token 级 ban——由 KV 层 tools-ad 遮挡修复（见下一条）；② lane hard stop 的 seal 分支语义写反导致 force-sealed worker 的宿主 slot 永不释放，残留 GENERATING 指向已删除 episode，裸传播误杀下一个无辜 episode（42 ms 内 bare abort，3/3 复现）——由 seal 成功即释放 slot 修复。两修后完整配对运行（seeds 777/12345/20260923 背靠背，thinking wire，Bonsai PQ2_0 / RX 6800）grouped 路径 3/3 全部干净收尾（9/9 清单，无泄漏），grouped 自此成为默认；无法 staging 的设备/形状自动保留单头标量路径，`GGML_VK_REROT_GROUPED_HEADS=0` 恢复 Br=1 供 A/B。
