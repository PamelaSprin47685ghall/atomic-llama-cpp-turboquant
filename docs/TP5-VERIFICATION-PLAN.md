...
2. **严禁越权**：任何涉及启动真实 GPU 推理、加载大模型、调用 Vulkan 硬件队列或触碰物理设备的动作，均严格受 `AGENTS.md` 真机安全门约束，必须等待用户明确下达指令后方可执行。
3. **只读 Engineer 边界**：本规程由只读 Engineer 梳理并提交，本阶段不执行任何硬件命令、不修改源码、不私自开启进程。
...
## 二、前置条件与硬件安全门（引用 AGENTS.md）

在执行任何真机测试前，必须严格执行以下前置安全门禁：
...
| Q sidecar 轮询与归约 | `ggml/src/ggml-vulkan/ggml-vulkan-collective.cpp` | L4482–L4526 | 区分 `late_sidecar_f16`（轮询 `bcast_host[4]`）与 F32（轮询 `status[6]`），并记录 `sidecar_wait_us`。 |
