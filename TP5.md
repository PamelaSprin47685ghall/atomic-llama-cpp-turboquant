- RELAY 的 host transport 已使用固定 storage descriptor，自身无需 imported-host BDA；默认不为消除 `RADV_DEBUG=nobolist` 警告而牺牲其它 Vulkan kernel 的设备能力。但显式设置 `GGML_TP5_ISOLATE_BO=1` **仍会在 VkDevice 创建前关闭整个设备的 BDA、descriptor indexing 及 coopmat2 派生能力**，使 RADV 可以不启用 global BO list；这是设备级取舍，不能当作仅 TP5 资源隔离。历史 52.47 tok/s 运行本身也是这些设备能力仍开启、RADV 拒绝关闭 global BO list 的状态下取得的。
...
**七行 target 验证的成本定位（2026-09-24，仅定向诊断）**：在同一 n=6 单卡草稿／五卡目标的两个正确 171-token 响应中，临时扩展既有 `GGML_TP5_MTP_PROFILE` host 相位记录，复测回复首个 target 七行图 `build=2.25ms、alloc=102.95ms、graph_compute=156.67ms`，共 **262.04ms**；后续 48 次七行重放 `alloc=0`、graph_compute 均值 **59.18ms**，完整 host 图均值 **59.21ms**，对应 target 周期均值 **66.19ms**，图完成之外最多约 **7ms／轮**。首轮额外约 **203ms** 包括 allocator／图首次执行，无法从源码静态归咎于 D2H；只改图后 logits 拷贝即使消尽剩余 7ms／轮，也不足以省出所需 ~0.61s。仪表源码已撤回，证据日志 `/var/tmp/tp5-mtp-n6-verify-host-profile.log`；带 profiler 的绝对吞吐仅作成本归因，不作为晋级结果。

**测量边界**：2026-09-23 02:39–08:26，watchdog 自动 PCIe 复位后 card2 性能一度只有 8.25 TFLOPS，其间取得的绝对 tok/s 和候选排序全部作废，不能拿来证明增益。五卡进入 manual 并选择最高 DPM 档后，独立 F32 ALU 探针观察到各卡 17.08–17.22 TFLOPS；card2 的静态 `pp_dpm_sclk` 表仍可能显示 1200MHz，**静态标称值不是负载算力**。不要为凑齐表面频率擅自复位或无限制 OverDrive。原 `scripts/reset-gpu-pci.sh check-clocks-only` 可锁定各卡 DPM；当前受控恢复助手在每次单卡复位成功后也必须恢复并核验 manual＋最高档。模型单次预热＋一次实测：171-token `1..60` 完整正确、自然停止，server decode **54.45 tok/s**（3.140752 s），完整客户端请求 **35.70 committed tok/s**（4.789839 s，含 prompt 1.453185 s）。这是当前资格与题型的**观察值**，不是所有请求的最优证明；原始响应、二进制/映射 DSO 哈希和设备身份来自 `/tmp/tp5-retest-highclock-smoke.json`（commit 前 dirty-tree 构建，二进制 SHA256 `8608fdee2737c5e1dac81068f0e1b5a209d3e5fa0107557b536d03217d9163ca`）。

**独占五卡复验与无收益臂（2026-09-24）**：临时调度器探针把首轮图的 ~103ms `alloc_graph` 分为 split ~6ms、reserve ~5ms、gallocr tensor bind **95–123ms**（约 8.3k 图节点／18k 绑定）；`ggml_backend_sched_compute_pool_set_retain_capacity(true)` 显式 opt-in 后 bind 仍 ~100ms、VRAM 多占约 46MB，故该实验撤销；探针源码亦已撤销。无 profiler、同二进制、每次仅一台 `llama-server` 的 n=6／7／8 同题 171-token Decode 两次复测分别 **72.75／72.83、72.64／68.83、72.61／70.20 tok/s**；仅为顺序探索、非 ABBA，n7/n8 没有晋级证据。`--spec-draft-backend-sampling` 在 n6 三次完整正确回复中草稿成本 ~0.44–0.49s（原 CPU 选项 ~0.42s），亦未晋级。所有探索不改默认。一次约 50s 的 Decode 异常发生时**两个诊断服务器误并行占卡**，GPU ALU 1s 五卡仍 17.79／16.56／18.42／17.37／17.36 TFLOPS、无内核错误；清掉多余进程后 n6 恢复 72–73 tok/s，**异常数据无效且不是本次降频证据**。原始日志 `/var/tmp/tp5-mtp-n6-verify-host-profile.log`、`/var/tmp/tp5-mtp-n6-sched-alloc-profile.log`、`/var/tmp/tp5-mtp-n6-gallocr-bind-solo.log`，顺序实验 `/var/tmp/tp5-mtp-n6-n7-n8-decode-pilot.json`。

**故障恢复更新（11:20 现场后）**：旧版 watchdog 曾在内核已成功 mode1 reset card5 的同一秒对该卡再做 PCI 热摘；虽单卡 SBR 成功并在真实复位后确认 manual＋最高 DPM index 2，该重复热摘风险不能接受。现服务保持 enabled/active，按 BDF 记录 ring timeout，优先等待驱动自动恢复；成功时只恢复该卡频率，90 秒无成功（明确失败 30 秒）后才在排空句柄和持久取证条件下考虑单卡 PCI 恢复。TP5+MTP 的设备端 hidden handoff 缺 native per-rank local copy，显式开启现在于 GPU graph 前 exit 1；不能用该不支持模式的崩溃推断正常 MTP 数值路径。RELAY/F32 本地草稿同进程 warmup＋四次完整 171-token 请求通过，committed **35.71–36.48 tok/s**；真实 card5 SBR 后再跑单次完整请求 **35.02 tok/s**，详见 `docs/TP5-MTP-EVIDENCE.md`。这些没有 >100 tok/s 的性能证明；当前 MTP 仍仅 opt-in，新的 watchdog 自动恢复分支有 mock 与旧现场证据但未故意重演物理故障。

**恢复脚本复核（23:43）**：用户报告自动恢复频率未生效后，复用原有五卡 Vulkan F32 ALU 1 秒探针，card1..5 为 **17.750／16.537／18.425／17.414／17.373 TFLOPS**；安装修复后复测 **17.847／16.523／18.425／17.416／17.367**，与原始同探针 **18.027／16.531／18.426／17.331／17.363** 同档。card2 静态表显示最高 1200MHz，但实际 ALU 没有旧 8.25 TFLOPS 腰斩；不可据此断言 171-token 纯 Target 客户端耗时应等于另一请求长度的 40–50 tok/s。旧 watchdog 仅在有 BDF 故障日志时恢复 DPM，驱动重新绑定而无 ring timeout 时即使回到 `auto` 也不会触发；新增轮询仅针对 `auto` 的非 PCI clock-only 恢复、跳过仍在等内核复位的卡，合成回归先红后绿。助手与 watchdog 日志现明确只证明 manual DPM 策略写入，**不以 sysfs 静态频率冒充实际算力**。`test-gpu-recovery.sh` 与 `test-gpu-recovery-mock.py` 通过；经授权安装器部署后 root-owned 源码一致，watchdog enabled/active，未故意触发物理复位；自动恢复触发分支仅由合成测试证明。

**MTP+LateBind 限定实验（未晋级默认）**：早期 F32-Q 路径先遭跨 site WAR 误拒，再于 RELAY generation 发布失败并在清理时 SIGSEGV；这些旧日志仍是事故证据。当前源码改为每 site 校验 WAR、`resume_norm` 支持 RADV 的 4/8 subgroup，并在释放 target/draft 前同步两者。`common_tp5_apply_env()` 仅允许五张不同 RX 6800、RELAY/F32 且显式 `--tp5-latebind exact` 的 MTP 实验；其它 MTP LateBind 仍在 GPU 图前拒绝。先前只捕获单行的 HC 生产者现对 2–4 行使用逐 token 索引，五卡完整 `1..60` 预热＋重复均为 171 token、自然停止；重复请求 target 定义 `late=46`，而不是先前仅单行首图 `late=47`、稳态 `late=0`。同进程 `1..10`、`13×17`、`1..60` 以及流式取消后的短、长请求均正确；内核无新增 GPU fault。**速度未晋级**：单次重复完整客户端 exact **34.81** 对同二进制保守臂 **34.98 committed tok/s**，稳态 target 中位 **49.186ms** 对 **46.782ms**，非配对烟测不能宣称净差异，更远未达到 >100。证据 `/var/tmp/tp5-mtp-multirow-{exact,baseline}-smoke.json` 和对应 `_logs/`。CPU-only 非法容量初始化在最新 server 清理路径仍精确 exit 1；11:20 GPUVM 资源寿命的最终失效点未证明，详见 `docs/TP5-MTP-EVIDENCE.md`。

**LateBind 分段瓶颈复核**：同样完整正确的 171-token 模型会话显式打开 `GGML_TP5_PROFILE=1` 与逐 stage 账，exact 48-stage target 图两次合计 **42.51／44.30ms**，其中 Y ready 合计 **36.77／38.65ms**、CPU 数据约 **1.15／1.12ms**；另一个独立的 Q sidecar 等待在第二张图达 **4.510ms**（46 个 site 多数每处约 85μs），真正 Q 归约仅 **0.113ms**。先前 reference 剖面两图 **40.31／40.15ms**，Y ready **39.28／39.18ms**。因此 exact 可减少部分 Y ready，却在 Y 发布后重新串行等 GPU Q，当前没有净加速；“把 CPU Q 加法再省数百微秒”不足以补上 4.5ms。数据 `/var/tmp/tp5-mtp-multirow-exact-stage-profile.json`，profile 扰动下的图耗时不可与无 profile 的客户端 tok/s 直接相减。

**GPU 时间戳锁定当前临界路径**：另两次独立、完整正确 171-token 会话在第 30 条 48-stage chain 注入一次 GPU timestamp（有屏障扰动，不是吞吐基准）。保守 reference 的 rank0 图 span **41.138ms**：本地 compute **35.906ms**、P2 **4.508ms**、跨 stage gap **4.32μs**；五 rank compute **32.665–35.906ms**。exact 的 rank0 图 span **44.158ms**：compute **34.186ms**、P2 **1.385ms**、独立 late-pre/Q **7.891ms**、gap **6.16μs**。首 stage rank0 compute **3.844ms**，其它四 rank **2.168–2.210ms**，有负载偏斜；但即使消掉该差异也不足以达到完整客户端 >100。对应原始响应和逐卡时间 `/var/tmp/tp5-mtp-gpu-timed-{reference,exact}30.json` 与 `_logs/`，解析 `/var/tmp/tp5-mtp-gpu-chain30-comparison.json`。单链跨进程测量不是性能配对；它排除了“只修 CPU 归约或 queue 空档就能达标”的归因，后续需要降低实际量化矩阵/模型算术成本并同步降低 MTP draft 与 prompt 墙钟。

**两项未晋级的单因素探针**：MTP draft 从 Vulkan0 移到 Vulkan2 后完整 `1..60` 重复为 **34.88 committed tok/s**；默认禁用的 Vulkan MMVQ 改为 auto 后为 **35.40**，二者均为单次非配对烟测、没有足以晋级的吞吐收益；后者还会对权重执行不同的 Q8_1 activation contraction，不能说参考数学逐位不变。证据 `/var/tmp/tp5-mtp-{draft-vulkan2,mmvq-auto}-smoke.json`，试验变体未保留为产品默认。**端到端硬约束**：171-token 请求若要 >100，完整墙钟必须 <1.71s；该次 MMVQ 重复 `prompt_ms=1708`、`predicted_ms=2926`（server 自报），单 prompt 已接近总预算；decode-only 优化不能单独满足此请求的验收。

**Q5_K/Q6_K 原生浮点 GEMV 行瓦片试验已回退**：仅对 RADV RDNA2 standalone F32 GEMV 试过 `NUM_ROWS=2→4`，保留 fused GDN/QSA 的 `rm_kq=2` 守卫，不曾粗暴关掉 fusion。Vulkan0 Q5/Q6 CPU oracle 26/26 通过；完整五卡 `1..60` 两次输出均准确、自然停止。插桩单链 rank0 compute **35.906→35.046ms**，但完整客户端 **35.22→35.04 committed tok/s**（两个独立进程，非可宣称的提升）；非插桩重复为 **35.26**，也无明确净收益。源码已还原原 `rm_kq=2`，避免扩大其它 Q5/Q6 工作负载风险；证据 `/var/tmp/tp5-mtp-q5q6-rows4-{smoke,timed30}.json` 及 `_logs/`。缩小 GPU 内部单阶段 0.9ms 不等于满足 <1.71s 的完整响应门。

**Prefill 源码边界与实测优先级**：`delta-net-base.cpp::build_delta_net` 对非单 token 在 `fused_gdn_ch=1` 时用 `ggml_gated_delta_net`，其 Vulkan kernel 按 T 逐步递推；普通 chunking 不会自动保留 TP5 非均匀 GDN head map。现有 `qwen4_gdn_segment_delta` 只在 `ggml_vk_can_fuse_gdn_segment` 的单行图形状守卫下接入，不能简单替换多行 prefill。但同一 27-token 真机 GPU optrace 显示 `GATED_DELTA_NET` 每 rank 约 **35×124μs=4.34ms**，不足总 prefill GPU 时间约 600ms 的 1%；即使把递推本体免费化也省不了 5ms，**WY 不应作为 >100 的优先路线**。该图的 Q5/Q6/Q8 量化投影才是已测大头；源码几何试验和完整客户端证据分别记在本节，不把 shader 局部计时冒充墙钟收益。

**Prefill Q8 瘦矩阵精确降层（已实现，尚无客户端净收益证明）**：真机 `GGML_VK_PERF_LOGGER=1` 逐 op 发现 27-token prefill 的 `Q8_0×F32`、K=10240、M=4/320 两种矩阵均被普通 GEMM 以 **1.26–1.30ms／1.39–1.46ms** 执行。RADV RDNA2 上仅在无 fusion、连续且对齐、单一权重矩阵、N=19–32、M≤320 时把物理等价的 `[K,N]→[K,1,N]`、`[M,N]→[M,1,N]` 作为 batched GEMV 送既有 F32 shader；源权重不改、没有 Q8_1 activation 近似。五 rank 同形 GPU timestamp 变为 **32–39μs／347–386μs**，证明局部算子成本大幅下降，但 profiler 关闭 replay 且扰动执行，**不能据此宣称端到端提速**。两个真实形状的 Vulkan0↔CPU 矩阵 oracle 通过；同进程 warmup+4 次完整准确 `1..60` 客户端 **35.49／36.25／36.08／35.99 committed tok/s**（均值 35.95；旧独立四次均值 36.03），无可认定客户端净收益。另有完整 `1..100` 292 token **41.69 committed tok/s**，混合短计数／算术及两次串流取消后的后继请求正确。证据 `/var/tmp/tp5-skinny-quant-prefill-profiles.json`、`/var/tmp/tp5-mtp-skinny-q8-320-{repeats5,count100}.json`、`/var/tmp/tp5-mtp-skinny-q8-mixed-cancel.json`。优化保留为已验证的物理 GPU 工量收敛，不晋级为 >100 或生产净收益声明；后续仍需降低 GPU target/draft 与 prompt 全链墙钟。

**Prefill Q6 同一几何（已实现，仍非端到端净收益）**：新增只针对 RADV RDNA2 上无 fusion、连续且对齐的 `Q6_K×F32`、M≤128、N=19–32 的共享权重 batched GEMV；没有额外 Q8_1 activation，`M=128,N=27,K=2560` Vulkan0↔CPU oracle 通过。真实五 rank、92 次 M=128 层的普通 GEMM **319–473μs／次**降至 **47–48μs／次**；70 次 M=48 层 **442–516μs／次**降至 **24–27μs／次**。同一仪器会关闭 replay，不代表客户墙钟；完整正确的 171-token 单次重复仍为 **35.37 committed tok/s**。原始 `/var/tmp/tp5-skinny-quant-prefill-profiles.json` 与 `/var/tmp/tp5-mtp-skinny-q6-prefill-smoke.json`，未声称客户端收益或 >100。

**Host prefill 与更深 MTP 探针（未晋级）**：`GGML_TP5_MTP_PROFILE=1` 门控的 27-token `process_ubatch` 实测：同进程重复 target 图 build **1.88ms**、`ggml_backend_sched_alloc_graph` **106.29ms**、输入设置 **0.04ms**、`graph_compute` **241.93ms**，总计 **350.28ms**；draft 的 27-token 图 **3.26ms**。该次完整正确的 171-token 重复为 **35.67 committed tok/s／4.794s**；server `prompt eval time=1618.74ms` 从 slot 开始到**首次生成**，不能当作 1.62s 纯 prefill GPU 时间。进一步 server 绝对时戳确认首次生成**先于**第一轮投机验证，故此前把 `prompt_ms` 归给首轮 verify 的猜测不成立。临时 `llama_context::synchronize` 探针仅 **12–24μs**，完整 `decode_impl` 比上述图相位多约 **1ms**；因此未保留这两处无价值插桩，保留可选图分账。独立单因素把 `--spec-draft-n-max` 从 3 改 6（未改产品默认），同一完整准确输出仍只有 **32.19 committed tok/s／5.312s**，比 3-token 臂差；重复稳态每轮 6-draft 约 **33ms draft + 66ms target + 2.5ms catch-up**，虽单轮吐出 7 token、server decode 从 **57.35→61.77 tok/s**，首次生成之前 server `prompt_ms` 却从 **1619→2337ms**，不能用 decode-only 数字晋级。两臂独立烟测，非配对净差声明；证据 `/var/tmp/tp5-mtp-prefill-sync-phases.json`、`/var/tmp/tp5-mtp-six-draft-smoke.json` 与各自 `_logs/`。实验用 runner 副本已删除，watchdog 与数值语义未改。

**单-cell recurrent reset 真瓶颈已修（仍未到 >100）**：额外的首次生成绝对时戳把同进程第二请求的 `slot.mem.seq_rm(seq=0,p0=0)` 单独测为 **907–935ms**，其中 target **约 907–913ms**、draft 仅 **约 0.1ms**；48 层／3 个回滚快照的 `clear_hand_row` 单次按 tensor 同步上传，成本都在逐 tensor 写（ready／末尾 sync 各约 0–1μs），不是 host graph build 或第一次 verify。目标机实际为 **size=1、n_seq_max=1、非 grouped、专用 RS buffer 一份（meta 内含五张卡）**。仅该独占布局，在清理前保留 scheduler fence、以一次 `ggml_backend_buffer_clear` 零化全部 R/S/PLE 与 rollback plane，清理后再 fence；多 cell 与有 brain 的 grouped/RERoT 仍逐行清理，不能覆盖旁人的状态。真机相同 warmup+repeat 单次 reset 由 **907ms→12.8ms／16.8ms**，完整准确且自然停止的两次 `1..60` 由此前未改的独立烟测 **35.60／37.28 committed tok/s** 改为 **44.33／46.05**（每次 171 token，完整客户端 **3.857／3.713s**）；server 首字前阶段从约 **1.62–1.66s→0.71–0.75s**，稳态 decode 仍约 **57–58 server tok/s**。移除所有临时分相日志后的最终二次完整复测仍为 **44.11／45.38 committed tok/s**，另有完整准确自然停止的 `1..100` **292 tokens／5.979s = 48.84 committed tok/s**。多组独立小样本不是统计晋级或全任务证明，但 source-level 因果与同进程 reset 时间直接一致；首个空 cache 的 warmup 本不触发旧瓶颈。证据 `/var/tmp/tp5-mtp-{slot-phase-gaps,recurrent-clear-phase,hand-shape-baseline,bulk-clear-smoke,final-bulk-clear-repeats,final-bulk-clear-count100}.json` 和各自 `_logs/`。临时 server／common／recurrent 分相日志已删除，只保留可选 host graph 分账；没有关 watchdog、缩短上下文或改变数值公式。171-token >100 仍须 <1.71s，眼下最佳观察约 46.05，剩余是 target/draft 计算与 prefill 图的真实墙钟。

**后续定向退选与测量隔离**：在 bulk-clear 后只把 draft horizon 3→2 的真实五卡独立烟测，171-token 完整正确自然停止但只有 **39.27 committed tok/s／4.355s**，server 稳态 `draft≈9ms + target≈45.5ms + catchup≈2.3ms`、每轮仅 3 token，不如 n3；证据 `/var/tmp/tp5-mtp-draft2-clean-smoke.json`。先前 `/var/tmp/tp5-mtp-draft2-after-bulk-smoke.json` 的 2.35 tok/s **完全作废**：独占服务管理器一度回报 profiler 已停止，同 PID `427508` 实际仍在运行并与 n2 模型同时占用五卡；发现后再次显式停止并通过 `pgrep` 核实无推理进程。随后同一纯 ALU 探针实测 card1..5 **18.011／16.555／18.424／17.369／17.362 TFLOPS**，与此前校准结果同档，无 GPUVM/新 reset；控制进程重跑才得到可信 n2 结果。另 `GGML_VK_PERF_LOGGER=1` 会关闭重放并把 target graph 的 48-stage op 组 GPU 计时扰动到 102–120ms，**只可用于排序热点、不可用于原生 tok/s**：定向采样中 Q5_K 多列 GEMV 与 IQ4_NL 专家乘为前列，具体 `/var/tmp/tp5-mtp-target-op-profile.json`。其候选 Q5_K 专用 shader `calc_superblock` 已在内层多 token 列循环外解包权重，不能再把“上移解包”重做一遍；优化须凭无 profiler 的完整客户端结果晋级。

**全行 GDN 融合试验已否决并撤销**：临时 `GGML_VK_GDN_FULLROW_FUSION=1` 让 active-count=0、K4 快照的 4 行验证图进入既有 multistep GDN shader。Vulkan0 合成图对 CPU 的输出／快照、异构 QK head map、无效尾行全部通过；独立 profiler 也确认合成图是 `GDN_SEGMENT`，并非测试未命中。但五卡真实 MTP 图的首次完整 `1..60` 请求立刻输出乱码（HTTP 200、`completion_tokens=192`、仅 11/536 draft accepted，客户端 14.65s），五 rank 每图 dispatch 从原 **4115→3791**，说明实际融合被选中而合成 parity 不足以保证模型图语义。原始失败响应与日志 `/var/tmp/tp5-mtp-gdn-fullrow-smoke.json`、同名 `_logs/`；未见服务残留，watchdog 仍 active。不得以减少 dispatch 或合成 CPU/GPU parity 称其正确／提速；生产不启用，源码已移除试验开关和多快照融合扩容，保持已验证的显式 active、K1 入场条件。回退并重建后两次同进程完整准确 `1..60` 为 **43.86／46.55 committed tok/s**（171 token／3.899、3.674s），证据 `/var/tmp/tp5-mtp-after-gdn-rollback-smoke.json`；未证明具体多行失真根因，也未满足 >100。

**七行 K7 GDN 复验同样否决，不能重复晋级**：在上述旧失败臂之外，临时扩容 full-active 七行／七个回滚快照并写回融合 shader 的 state 与 snapshot；Vulkan0 上 C7/K7 的全行、部分行、异构 head map 对 CPU 的合成 oracle 全过，性能日志确认真实五卡模型执行了 `GDN_SEGMENT`。但固定 seed 的 `1..5` 短请求返回 `Count1u\n.CountA11`（HTTP 200、9 token），同 prompt、seed 的原生 GDN 对照正确返回 `1,2,3,4,5`（10 token），原始响应对照 `/var/tmp/tp5-mtp-gdn-k7-fused-rejected.json`。这是实模数值失败，不能拿合成 parity 代替；七行融合守卫及 shader 写回已撤销。保留 K7 native rollback-bank oracle；回退重建后该测试全过，同进程未开 profiler 的 `1..60` 预热及后两次均逐字正确、自然停止、171/171，后两次 server Decode **72.02／72.58 tok/s**（`predicted_ms=2374.478／2355.920`），证据 `/var/tmp/tp5-mtp-gdn-k7-native-postrollback.json`。watchdog active、内核无新增 warning、无残留 server；**仍未达到 100 Decode tok/s**。此路不要继续靠放宽 snapshot/active guard 重试。

**随试验发现的真实 partial-row GDN 缺陷已修**：默认 generic Vulkan GDN 原先无 active-count push constant，异构 head map 的 K4／C=4／A=2 合成测试在禁用融合后**先红**（两种不同未激活尾行污染了输出快照），不能用原先 K1 测试冒称任意多快照正确。现 generic shader 仅执行已验证的 active 行，并相对 active 行反排 rollback slot；生产 graph 的 `n_written` 同样取已执行行数而非容量，阻止拷贝未写快照。相同的 Vulkan0↔CPU oracle 与无效尾行双跑现全过，`test-target-capacity` 全过；这不放开 `evaluate_target_capacity_admission` 对 PLE/QSA/GDN 的拒绝，也不恢复不正确的 K4 融合。五卡参考 MTP 同进程两次完整准确自然停止的 `1..60` 为 **44.31／44.88 committed tok/s**（171 token／3.859、3.810s），证据 `/var/tmp/tp5-mtp-after-gdn-active-fix-smoke.json`；正确性修复，不是性能晋级或 >100 证明。

**Q5_K MMVQ 多列解包上移也已退选**：虽然源码的 `mul_mat_vecq.comp` 对 4 列逐列调用 `repack4/get_dm_scale`，试验把同一块权重解包移到列循环外后，Vulkan0 `q5_K×f32 n=2/4` 对 CPU 全过，但 RADV 最终 pipeline 的 hash **273202172**、code **23296 B**、VGPR **40／无 spill**、指令 **4305** 两臂完全相同；同机单 op `4096×14336×4` 为 **81.52→81.58 μs**（试验→原版），没有可归因的执行收益。未开 profiler 的真实客户端试验完整准确 `1..60` 两次 **43.32／45.47 committed tok/s**，恢复原 shader 后 **43.33／45.63**；证据 `/var/tmp/tp5-mtp-q5-mmvq-hoist-smoke.json` 与 `/var/tmp/tp5-mtp-post-q5-revert-smoke.json`。该客户端参考臂实际上设置了 `GGML_VK_DISABLE_MMVQ=1`，不可把客户端速度当作 MMVQ 代码的性能归因；真正的单 op 对照和 RADV ISA 均无收益。源码已彻底回退；不能把 GLSL 文本内循环的重复解包误报为 RADV 最终 ISA 的四次实际显存读取。IQ4_NL 跨 token expert 共享只有在真实路由重叠率经过采集后才有可验证收益，当前不以猜测路由比例新增 grouped kernel。

**四行 GDN/QSA 投影融合也已退选**：限定 `GGML_VK_PROJECTIONS_MULTIROW=1` 试验把四笔校验输入分派给同一融合投影 shader 的四个 workgroup 行，分别验证了 GDN/QSA、Q5_K/Q6_K/BF16、MMVQ 与原生 F32、多次命令重放的 Vulkan0↔CPU oracle；五卡真实模型日志也见到两族 `fused=1 mmvq_mask=0`（此参考模式明确 `GGML_VK_DISABLE_MMVQ=1`）。48-stage rank0 dispatch **4115→3971**，客户端完整准确同进程 `1..60` 两次反而为 **42.77／44.73**，同一二进制关闭臂 **44.43／45.80 committed tok/s**，没有净收益。当前 fused shader 每 token 一个 WG，重复读取同一投影权重；原本独立 batched F32 GEMV 已在多列间复用权重，减少 dispatch 不能作为性能替代。源码、shader ABI 与测试的试验增量均已撤销；回退后完整准确两次为 **44.25／45.33**。证据 `/var/tmp/tp5-mtp-projection-multirow-smoke.json`、`/var/tmp/tp5-mtp-projection-control-smoke.json`、`/var/tmp/tp5-mtp-post-projection-revert-smoke.json`；本臂仍远低于 >100。

**共享解包权重的四行投影也未晋级**：进一步让单个 WG 将 Q5_K/Q6_K 权重解包一次供四笔 F32 token 使用，QSA 的 BF16 输出保持逐行原算法和输出次序；限定四行 opt-in，GDN/QSA CPU↔Vulkan0 参考数学位级对拍通过，真实五卡 `fused=1 mmvq_mask=0` 且客户端两次正确自然停止。但打开臂 **42.95／45.87**，同一二进制关闭臂 **43.79／45.43 committed tok/s**，一臂更慢、一臂仅噪声量级，未见跨两次的一致净收益，远不足以靠近 >100；故没有留开关、shader ABI 或多列测试负担。原始记录 `/var/tmp/tp5-mtp-shared-projection-smoke.json` 与 `/var/tmp/tp5-mtp-shared-projection-control-smoke.json`；完全回退并重建后再测得正确自然停止 **43.75／46.27**，证据 `/var/tmp/tp5-mtp-post-shared-projection-revert-smoke.json`。进一步减小数千个独立 target dispatch 的收益不能靠此单个投影微融合推断。

**MTP 草稿卡位置的定向烟测**：同一已回退二进制只把 `--spec-draft-device Vulkan0` 改为 `Vulkan4`，独占服务 `1..60` 两次 171 token 正确自然停止，完整客户端 **43.81／45.05 committed tok/s**，未见有别于原 Vulkan0 约 44–46 的收益，更不接近 >100；`/var/tmp/tp5-mtp-draft-rank4-smoke.json` 保存 CLI、设备身份、五 rank 路由和响应。此为顺序两次烟测而非交错配对，仅排除显著单因素收益，不据此比较小幅差异。未改默认配置。

**MTP 草稿模型单设备真实装载（已证净收益，仍仅显式 opt-in）**：此前即使 `--spec-draft-device Vulkan0`，draft 仍从 target 继承 `split_mode=tensor`，启动提示「TP5 plan expects 5 tensor devices but got 1」，而 CPU sampler 因该 tensor 标签无法使用 GPU 后端。现只有 `GGML_TP5_MTP_DRAFT_SINGLE_GPU=1` 且草稿设备表恰为「一个真实设备＋终止 null」才在**草稿模型装载前**改为 `LLAMA_SPLIT_MODE_NONE`；target 仍原五卡 RELAY/F32 参考数学，不改 recurrent/snapshot/校验。此臂显式 `--no-spec-draft-backend-sampling` 固定原有 CPU top-k 提案；零阈值 GPU greedy 也能正确走通，但一次烟测略慢，正阈值继续 CPU 候选概率判决。五块未剖析 ABBA（每臂每块各两独立进程、各预热再一次 `1..60` 171-token 完整精确自然停止）原继承模式 **44.33**→草稿单设备 **46.47 committed tok/s**，配对差 **+2.14**、区间 **[+2.06,+2.22]**；原始逐请求日志 `/var/tmp/tp5-mtp-draft-single-gpu-abba5.json`。`1..100` 292-token 正确烟测 **51.44** vs **47.71**（顺序单次，非统计配对）。单进程短计数／13×17／长计数均正确；长流首 token 后取消，随后短长两请求逐字正确且自然停止，证据 `/var/tmp/tp5-mtp-draft-nosplit-mixed-cancel.json`。此优化幅度不足以达 >100，不默改 MTP 默认；生产脚本与显式 CLI/env 优先级均不动。

**草稿 6-token horizon（同样限 opt-in）**：上述单设备草稿及 CPU sampler 固定，只把 `--spec-draft-n-max 3` 改为 `6`；五块完整正确 ABBA **46.53→52.03 committed tok/s**，配对差 **+5.51**、区间 **[+4.87,+6.14]**，`/var/tmp/tp5-mtp-draft-nosplit-n6-abba5.json`；独立 `1..100` 同进程两次完整正确自然停止 **55.65／61.39**，`/var/tmp/tp5-mtp-draft-nosplit-n6-count100.json`。更远的 7／8／10-token 独立烟测虽全文正确，却不稳健优于 6：10-token draft 热中位约 **83 ms** 对 6-token 的 **17 ms**，验证段亦 **104 ms** 对 **66 ms**，完整请求仅 **41.13／42.84**，故不外推「horizon 越大越快」；单次烟测见 `/var/tmp/tp5-mtp-draft-nosplit-n{6,7,8,10}-smoke.json`。这些计数负载接受率很高，并不证明其他提示的投机收益；**>100 committed tok/s、171 token <1.71s 仍未通过**。尚需目标图执行与草稿跨容量掉速两端结构优化，不能靠把 warmup／decode 独立速率当作用户墙钟。

**n=6 混合请求与中断恢复**：同一五卡服务按序完整 `1..10`（21 token／2.089s）、`13×17`（4 token／1.608s）、`1..60`（171 token／3.267s），均逐字正确自然停止；流式 `1..1000` 收到首个 `1` 后取消，再发 `1..10` 和 `1..60` 仍逐字正确自然停止（2.622／3.288s），`/var/tmp/tp5-mtp-draft-nosplit-n6-mixed-cancel.json`。非配对的原 n=3 单进程短响应曾 1.778s，故不能把计数长输出的 n=6 增益外推为短任务的普遍获益；MTP 与 horizon 继续 opt-in。

**n=10 退速诊断与门禁**：临时逐 draft 步计时（现已删）在未关 Vulkan replay 下看到每步 GPU decode enqueue 约 **0.15ms**（n6/n10 相同），CPU sampler 触发的 GPU 完成等待却由 n6 的约 **2.35ms** 增至 n10 的约 **7.5ms**；首步也是 1.68／2.18 对 5.69／8.93ms。故非最后两步才慢，也不能把 sampler 函数名误断为采样 ALU 根因。定向诊断原始 `/var/tmp/tp5-mtp-draft-n{6,10}-step-diagnostic.json` 与 `_logs/`；带计时请求不参与性能验收。试开现有 `GGML_TP5_MTP_MAX_CAPACITY=1` **HTTP 500 fail-closed**：资源层将单请求 27-token catch-up 解释为 `tokens=27,sequences=27`，超出一序列上界，不能为绕开 n10 掉速而启用；`/var/tmp/tp5-mtp-draft-nosplit-n10-capacity-smoke.json`。试开 `GGML_TP5_MTP_DEVICE_HIDDEN=1` 则在模型初始化前明确拒绝「TP5 native per-rank local copy unavailable」，无请求执行，不能越过保护来冒领 device-hidden 收益；`/var/tmp/tp5-mtp-draft-nosplit-n6-hidden-smoke.json`。两者都不是可用快路。

**最大容量门的进一步根因（未合入半修）**：`llama_ubatch.n_seqs` 是 token 的**序列集合数量**，而容量 ABI 的 `sequences` 是互异物理 seq，应取 `n_seqs_unq`。局部改正后同一 real-model warmup **仍 HTTP 500**：2-token 预热被 `n_outputs>0` 误分为 DRAFT，27-token 无输出预填被误分为 CATCHUP，不能靠 `n_outputs` 猜实际 phase；证据 `/var/tmp/tp5-mtp-draft-nosplit-n10-capacity-after-seq-fix.json` 及 `_logs/`。因还缺调用方明确的 prefill/draft/catchup 阶段契约及完整端到端证明，这个**单独修 seq 计数的中途补丁已撤销**，保持资源层关闭/失败态而不是默默放行。

**容量门下一个阻塞已由真机明确点名，禁止仅修阶段后强行放行**：临时同时使用互异物理 seq 和调用方 draft/catch-up 显式 phase，普通 prompt 维持 PREFILL；两 token warmup／27 token 预填先成功到达正式草稿步。但该图按容量 7、实际活跃 1 行执行时，`[predefined-coverage]` 捕获 81 个 dispatch 中 **46 个未分类**，`fixed_compute`／`dynamic_compute`／`state_writes`／`dep_boundaries` 未建立容量降层证明，Vulkan 主动报 `MTP draft active rows=1/7 rejected` 并 HTTP 500。错误后续的 catch-up frame 同样被拒，不是可用快路。证据 `/var/tmp/tp5-mtp-n6-capacity-phase-smoke.json` 及 `_logs/server_mtp-n6-capacity_smoke_60.log`；临时 phase API／physical-seq 补丁／测试现已完整撤销。仅有 phase 分类正确不等于 46 个 native dispatch 可在 `active<capacity` 下正确执行，不能跳过 coverage 守卫；这条 opt-in 继续 fail-closed，未拿它测或声称吞吐。
最终回退源码重建后 `test-target-capacity`／`test-mtp-workspace`／`test-rerot-recurrent` 3/3 通过；原生 n=6 的同进程两次 171-token 完整精确自然停止为 **52.46／55.46 committed tok/s**、3.260／3.083s，`/var/tmp/tp5-mtp-n6-post-capacity-revert-smoke.json`。这是验原路径的单臂烟测，不是容量臂收益或 >100 证明。

**原生 device-hidden 草稿交接未晋级（2026-09-24）**：为验证早期 `native per-rank local copy unavailable` 拒绝是否过于保守，临时仅对五卡 RELAY/F32 target＋单卡草稿开放同设备、全镜像 Meta RESULT 的 rank-local 借用；草稿 `mtp_h_input` 改为原生 Vulkan 驻留，并使预热 host 图失效。首版虽通过启动 preflight，却在短请求 deferred catch-up 的图输入拷贝处报 `device MTP hidden input has no native local copy`；定位并修正驻留之后，Vulkan4 上 warmup＋三次 **171-token 逐字正确、自然停止**的 Decode 为 **71.25／70.88／68.91／69.25 tok/s**，同设备 host 对照为 **61.42／69.77／70.62／68.47**。Vulkan0 隐藏交接的 warmup＋三次为 **63.95／70.95／71.81／71.92**；各组为顺序 pilot、非交错配对，不证明微小收益，更不达 100。原始响应 `/var/tmp/tp5-mtp-n6-hidden-native-vulkan{4,0}-pilot.json` 和 `/var/tmp/tp5-mtp-n6-gpu4-host-control.json`。此路需额外维护镜像 Meta/草稿图生命周期与取消恢复，收益门未过，**临时 Meta API、借用状态、图驻留修改与诊断现均撤销**；`GGML_TP5_MTP_DEVICE_HIDDEN=1` 继续在 target 加载前 fail-closed。回退后二进制的 `test-mtp-workspace`、`test-arg-parser`、`git diff --check` 通过，守卫服务启动实际以预期错误退出，watchdog enabled/active。正确输出只覆盖这些请求，不能当作全 logits、取消或长上下文的安全证明。

同一回退后二进制、五卡未开性能探针，Vulkan0 单卡草稿 n=6 的完整 `1..60` 预热＋两次精确自然停止均为 **171/171 token**，server `predicted_ms=2620.763／2334.224／2317.440`，Decode **65.25／73.26／73.79 tok/s**；首个样本仅预热，后二者为单进程顺序观察值。原始逐次响应 `/var/tmp/tp5-mtp-n6-post-hidden-rollback.json`；五卡服务已停、无残留 server、watchdog enabled/active、内核无新增 warning。这证明现有基准路径仍可用，**并非**原生隐藏交接获得 >100 或已改善。

**n=10 草稿慢速已隔离到真实 GPU 算子，而非容量门或 CPU 采样**：现有 n=6／n=10 单卡诊断里每个后续草稿步的 `llama_decode` CPU enqueue 同为约 0.15ms，但同步 sampler wait 分别约 2.2／7.5ms。`GGML_TP5_MTP_MAX_CAPACITY` 未开，故不能将 n=10 归咎于此前 opt-in 的 46 个未覆盖 dispatch；phase flip 也只在 catch-up→第 0 步发生。匹配短请求、带 `GGML_VK_PERF_LOGGER=1` 的 Vulkan0 单行图显示同形状 `MUL_MAT_ID_VEC q4_K m=640 n=10 k=2560` 两次投影由 n=6 各约 **75µs** 变为 n=10 各约 **1454µs**，`q4_K m=2560 n=4 k=5120` 从约 **128µs** 变为 **1170µs**；在采样器等待中确有 GPU 工作变慢，不应归为 host re-record。此前单卡 n=6 请求后 BDF09:00 VRAM **16.860/17.163 GB**，另一个独占 n=10 进程在该卡约 **14.950 GB**；这些记录未同步保存 Vulkan 设备枚举，不可仅用后续进程的索引映射证明那时的草稿卡归属。新的一对同配置独占试验在当前枚举（Vulkan0=BDF03:00）下，完成响应后 Vulkan0 VRAM n6 **16.861/17.163 GB**、n10 **16.400/17.163 GB**，但驱动 GTT 对照是 **25MB／985MB**（模型刚加载便约 **19MB／978MB**）；`GGML_VK_MEMORY_LOGGER=1` 在 n10 加载后把该卡逻辑 device-local 缓冲合计记为 **16.08GiB**，高于物理 VRAM 驻留约 0.9GB，不能仅凭 VRAM-used 小于总量就排除驱动内存迁移。尚未识别 GTT 对应的 buffer/page，更不能断言 Q4_K 权重就是被迁移者；现有数据不足以从相关性判定 20 倍 kernel 退速根因。profiler 会改变 replay 与执行时间，不能用这些 µs 当生产速度。只改为五卡 tensor-split 草稿、n=10 的独占未开 profiler 完整 `1..60` 预热＋复跑均逐字正确、自然停止、171/171；server Decode **53.14／59.77 tok/s**（`predicted_ms=3217.649／2860.745`，`draft_n=160`），`/var/tmp/tp5-mtp-n10-split-draft-pilot.json`。比单卡 n=6 的 73–74 慢、远未过 100；不晋级该路由，未修改默认代码。算子探针日志 `MtpN6DraftPerGraph`／`MtpN10DraftPerGraph` 只用于诊断，不是配对性能证据。两臂均为 `MUL_MAT_ID_VEC`：名称里的 `n=10` 是同一算子的 top-10 专家输出维，不是 MTP 草稿 horizon；direct GEMV 入场看的是 `ids->ne[1] == 1`（单 token），而非 `mul_mat_vec_id_direct_max_cols=8`。不得把 horizon 10 当作跨过该上限、擅自扩大 kernel 门限；n=10 退速机理仍待证。

**n=10 草稿位置、低比特与广播顺序定向退选（2026-09-24）**：生产 F32/RELAY 五卡 target 与单卡草稿的 n10 只把 draft 移到 Vulkan4，完整准确自然停止 171/171 的未开 profiler 两次重复仅 **52.02／51.96 Decode tok/s**（`predicted_ms=3286.983／3291.255`）。临时、显式 opt-in 的 Vulkan0+Vulkan4 草稿 **按层分卡**亦只有 **51.49／51.70**（`3321.339／3307.779ms`）；试验代码撤销并重建，无默认改动。将原草稿 Q4_K_M（2650.64MiB）一次 requantize 为 Q3_K_M（保留 Q6_K output，2047.02MiB）后，n10 在 Vulkan0 的两次重复升至 **69.94／70.56**（`2445.088／2423.307ms`，156/160 accepted）；n12 却降至 **63.21／62.70**（`2705.214／2727.130ms`）。进一步 Q2_K（1714.47MiB）n12 的草稿质量崩塌为 **146/324／145/324 accepted**，重复仅 **38.32／38.13**（`4462.854／4484.239ms`）。两份 requantized 文件只为试验，均已删除；Q3/Q2 的 SHA256 分别为 `d3cf667f347a760696e278de9ef874aaef02c0676de763e4b5e592f665e52bb9`／`9b335fa6cbc3c92cda3829485816a13eced10f057a6067a7a70412de3b6cac82`；可用 `llama-quantize --allow-requantize --leave-output-tensor` 从原始 GGUF 复刻。原始完整响应 `/var/tmp/tp5-mtp-n10-{vulkan4-single,layer2,q3-draft}-probe.json`、`/var/tmp/tp5-mtp-n12-{q3,q2}-draft-probe.json`。这些是顺序单臂探索，不足以比较 69.94 与原 n6 73 的小幅差值，但排除了到 100 的大幅收益，**不修改默认模型／草稿量化**。另一临时 `GGML_TP5_SEQUENTIAL_BCAST=1` 将 F32 Y 的 AVX2 求和后依次 `memcpy` 五张 BAR，五卡 RELAY mesh 50176 元素／8 轮全绿，未开 profiler 的 n6 完整重复只有 **69.77／70.60**（`2450.766／2422.214ms`）；顺序写并未突破既有五向直写，临时代码撤销并重建。`/var/tmp/tp5-mtp-n6-sequential-bcast-probe.json`；所有样本均满足精确内容／自然停止／171/171，不把预热的慢速当稳态收益。

**原 Q4 草稿仅分离 output head 的单因素试验同样退选**：临时指定两张 draft 设备的重复层全部在 Vulkan0、output layer 预期在 Vulkan4（目标五 rank 不变，仍用 F32/RELAY）；n10 同进程两次逐字正确自然停止 171/171，Decode **52.07／51.99 tok/s**（`predicted_ms=3283.837／3288.936`），没有比原 Q4 n10 的 52 左右有意义的提升。原记录的物理 VRAM 读数 BDF09:00 14.86GB／BDF03:00 16.34GB **没有同时保存该进程的 Vulkan UUID→BDF 映射**；后续进程的 Vulkan2／Vulkan0 枚举不能反证那次设备归属，更不能单凭 VRAM 证明 output layer 的物理驻留。此臂只证明指定参数下吞吐未改善。试验开关已撤销，原始请求 `/var/tmp/tp5-mtp-n10-output-head-separated.json`，**不是**有资格的 >100 结果。

同次枚举下的 n6/n10 完整 171/171 响应及五卡内存快照存于 `/var/tmp/tp5-mtp-n6-n10-physical-vram-control.json`；n10 单次是模型预热，`predicted_ms=3689.266` 不与上述历史正式复跑组成配对加速论证。
在同样的 n10 单卡草稿、五卡 target 下，单独把 `-ub 32→16` 或移除 `GGML_VK_DISABLE_HOST_VISIBLE_VIDMEM=1`，两臂刚加载后的 Vulkan0 GTT 仍各为 **978264064 bytes**；只做装载／驻留诊断，没有整请求配对性能结论。不能把这些开关称为已验证的 n10 加速方案。
恢复原始 Gallocr 并重编后，独占、无 profiler 的 n6 Q4_K_M 五卡 `1..60` 预热和两次复跑均逐字正确、自然停止、171/171；正式 `predicted_ms=2372.330／2340.143`，**72.08／73.07 Decode tok/s**，`/var/tmp/tp5-mtp-post-allocprobe-restored-unprofiled.json`。此数据证明仪器已撤除且原路径无数值破坏，仍非 >100 验收。
仅显式 opt-in 的 `--tp5-wire f16`（保留相同 F32 以外设置和 linear=1）也在独占五卡、无 profiler 的 n6 `1..60` 同进程预热＋两次重复中逐字正确、自然停止、171/171；正式 `predicted_ms=2419.207／2363.261`，**70.68／72.36 Decode tok/s**，`/var/tmp/tp5-mtp-f16-transport-reference-control.json`。这是顺序烟测而非随机化配对，但足以排除相对 F32 约 73 tok/s 的跨越 100 的巨大收益；不改变参考 F32 默认或把 F16 的正确计数误作相同数值数学。
**MTP 验收采样 CPU 账与同步简化退选**：临时 `GGML_TP5_MTP_PROFILE=1` 记录每次 target 后 `common_sampler_sample_and_accept_n`（含全量 CPU logits/grammar/sample），两条正确 171-token 请求分别约 **94.0／85.7ms**，25 个七行周期平均 **3.6ms/周期**，`/var/tmp/tp5-mtp-n6-host-sampler-profile.log`；profile 扰动令这些数值只用于归因，不能作未开 profile 净收益。把函数改为每批只调用一次 `llama_synchronize(ctx)`、每行走 `synchronize=false` 的 opt-in 代码试验仍在未开 profiler 的五卡整请求中仅 **69.93／72.01 Decode tok/s**（均逐字正确、自然停止、171/171，`/var/tmp/tp5-mtp-single-batch-sync-pilot.json`），不比恢复源码对照 72.08／73.07 有可证收益；而多行 getters 仍自行同步。改动和计时探针均已撤销，不能将一次批级 fence 当成达到 100 的关键修复。
**单行普通 Target 对七行 MTP 验证的真实同格式 GPU 链比较（仅诊断）**：同构建、五卡、F32/RELAY、`GGML_TP5_GPU_TIMING=30`，两次独占进程分别完整正确自然停止 171/171。单行纯 Target 的 rank0 48-stage chain：compute **20.422ms**、P2 **5.129ms**、total **25.564ms**；七行 MTP 第二请求的 rank0 同链：compute **46.257ms**、P2 **8.256ms**、total **54.523ms**，即整链约 **2.13×**、本地计算 **2.27×**、P2 **1.61×**。原始 `/var/tmp/tp5-one-row-chain30-gpu.log`、`/var/tmp/tp5-seven-row-chain30-gpu.log`。所以七行链相对七次单行有约 **3.28×** 的 GPU 链账面摊销，但权重只是在瓦片内复用，七列 FMA、GDN 顺序递推及各行可能选中不同专家仍需工作；P2 一轮协议也需传更多数据并等待最慢 rank。两臂顺序非配对、插 GPU timestamps、有普通 Target 与 MTP state-snapshot 路径差异；数字不能等同未插桩响应净吞吐或相减声称优化。
**不以“额外开销”掩盖草稿与 target 账（仅诊断）**：在一次独占五卡、F32/RELAY、n=6 同进程预热后，第二条完整正确、自然停止 171/171 响应 `predicted_ms=2437.917`；显式 profile 25 周期不重叠主账：草稿 **514.051ms**、target 验收 **1885.206ms**、catch-up **35.891ms**、handoff 0，合计 **2435.148ms**，余 **2.769ms** 是 ledger 起止与 server `predicted_ms` 边界差，不能外推为固定运行负担。草稿 150 个串行单步的内部计时：`llama_decode` 提交 **67.454ms**、`common_sampler_sample`（含等待 GPU 完成与 CPU 采样，**不是纯 CPU 时间**）**445.386ms**、取 hidden **0.099ms**、草稿循环其余 **1.112ms**；当前 GPU backend sampler 只搬 token id，但下一步仍依赖上一轮完成，不能从“一层”推出零时延。target **1885.206ms** 内 `llama_decode` **1790.150ms**、post-decode **95.056ms**；后者含 sampler clone **6.363ms**、七行验收采样 **87.058ms**、KV `seq_rm` **0.099ms**、已计 token 输出循环 **0.121ms**、其余 **1.415ms**。嵌套在 `llama_decode` 内的 25 张七行图合计 **1700.078ms**（build **2.238ms**、sched alloc **121.978ms**、graph submit+wait **1574.206ms**、set inputs **1.357ms**，另有时钟边界约 0.299ms），故 `llama_decode` 中图外的阶段约 **90.072ms**；这些**嵌套项不可再加到主账**，且图外项尚未细分为唯一根因。首次图 `elapsed=287.519ms`，后续 23 张同形图均值约 58–59ms；这是带日志的单请求账，不把先前另一进程的 54.523ms GPU query 时间逐项相减，也不声称删日志就快 0.6s。原始 `/var/tmp/tp5-mtp-n6-granular-ledger-{cycles,draft,target,graphs}.log` 与 `/var/tmp/tp5-mtp-n6-granular-ledger-responses.json`；所有临时细分探针已撤销并重编。
**相邻草稿 horizon n=7 退选**：相同五卡、原 Q4_K_M、参考 F32、无 profiler 的完整 `1..60` 预热后两次正式自然停止、逐字正确、171/171：`predicted_ms=2360.370／2359.163`，即 **72.45／72.48 Decode tok/s**，没有超过 n=6 的约 73，不能仅靠把单次验收的名义产出再加一枚来兑现局部 GPU 链加速。原始 `/var/tmp/tp5-mtp-n7-q4-unprofiled.json`。
**五卡 TP5 草稿对照与分片采样门**：不启用 `GGML_TP5_MTP_DRAFT_SINGLE_GPU`，显式设 `--spec-draft-device Vulkan0,Vulkan1,Vulkan2,Vulkan3,Vulkan4`，target／draft 各自初始化五 rank F32/RELAY collective，证明五卡草稿本已有执行路径；原样五卡 draft 完整正确、自然停止 171/171，两次无探针正式复跑 `predicted_ms=2985.592／3010.780`（**57.28／56.80 Decode tok/s**），低于单卡 draft 对照约 73，原始 `/var/tmp/tp5-mtp-n6-five-rank-draft-unprofiled.json`。启动时现有 `llama_context::set_sampler` 因 `SPLIT_MODE_TENSOR` 拒绝 backend sampler，记录 `using CPU`；这既非单卡0.514s账，也不能证实五卡草稿设计没有潜在收益。尝试仅放宽单序列 MTP 门即在初始化时报 `ggml-backend-meta.cpp:882: GGML_ASSERT(src_ss[0].axis != GGML_BACKEND_SPLIT_AXIS_0)`：五卡草稿的词表 logits 是 axis-0 分片，现有逐行 backend argmax 不能原样跨 rank 求全局最大值；已立即撤销该试验并恢复 fail-closed guard、重编 server。要在五卡草稿上做精确 GPU 采样需设计并对拍每 rank 局部 `(max_logit, global_token_id)` 的稳定 tie-break 归约／输出张量所有权，不能仅改一处条件；五卡 native per-rank hidden copy 的新实测与独立 opt-in 门见下文，不能把可用的设备端 hidden 交接等同于完成草稿／target 图融合。**现有五卡 draft 较慢是该路径的实测，不等于五卡草稿在修全这些契约后的性能上界。**
**五卡草稿同次周期账（仅诊断）**：五卡 draft 的第二次完整 171/171 响应 `predicted_ms=3008.029`，25 个 MTP 周期互斥主账：draft **869.705ms**、target **2000.113ms**、catch-up **135.433ms**、handoff 0，共 **3005.251ms**（起止残差 2.778ms）。同样开 cycle profile 的另一次单卡 draft 臂是 **514.051／1885.206／35.891ms**；两臂分进程非配对，只定位五卡执行和状态提交的代价，不能将差额称为 sampler 唯一原因。源码 `common_speculative_impl_draft_mtp::commit` 的 catch-up 计时段**确实又对 draft ctx 做一次 `llama_decode`**，不是纯 host 拷贝。五卡 draft 比单卡多付多 rank 图与 CPU 采样，设备端融合仍需处理 axis-0 分片词表与状态归属；per-rank native hidden copy 已按下文独立 opt-in 打通，但本身没有带来净加速，不能只删除 sampler guard。原始 `/var/tmp/tp5-mtp-n6-five-rank-draft-cycles.log` 和 `/var/tmp/tp5-mtp-n6-five-rank-draft-cycle-responses.json`。
**五 rank 原生 hidden 的完整修复与负性能门（非默认晋级）**：此前 `GGML_TP5_MTP_DEVICE_HIDDEN=1` 在 `common_tp5_apply_env` 一律拒绝，只验证了 target Meta→单 Vulkan 草稿的不兼容。两个独立五 rank Meta backend 共用进程级 Vulkan device；不加载模型的探针验证镜像 F32 RESULT→CARRY 逐 rank 预检和五块 GPU 读回均位级一致，原始 `/var/tmp/tp5-meta-hidden-copy-preflight.txt`。现仅在目标设备与 draft **相同有序五张 RX6800**、TP5 `qwen4exp-af`、tensor split、RELAY/F32 时允许显式 opt-in，原始 `enable_predefined_hidden` 仍对真实缓冲逐 rank dry-run，非匹配布局继续在模型启动前拒绝。真模型初次 catch-up 由原图调度把 `mtp_h_input` 置于 CPU 而 fail-closed；现只在启用 device hidden 的 MTP ctx 图定义时将该输入钉到自己的 Meta hidden-store executor，保留其余输入调度不变。修后同进程短题 `1..5` HTTP200 正确，三次 `1..60` 全部逐字正确、自然停止、171/171；流式请求断连触发 server `cancel task`，随后同 server 完整 171/171 再次正确。**实际吞吐退选**：无 profiler 正式两次 `predicted_ms=3151.723／3147.763`，即 **54.26／54.32 Decode tok/s**；同二进制随后五卡 host-hidden 对照 **56.70／56.80**（`/var/tmp/tp5-mtp-n6-five-rank-host-matched.json`），不属于 >100 候选。原始 `/var/tmp/tp5-mtp-n6-five-rank-native-hidden-qualified.json`；GPU 采样仍在 CPU fallback，设备端 hidden 与 target/draft 全图融合不是一回事。保留显式 opt-in 仅作为已验证的五 rank 本地隐状态通道，不更改生产默认，后续若无联合优化收益可删除。
**本次五卡切换负控制**：`GGML_TP5_MTP_DEVICE_HIDDEN=1` 搭配单卡 draft、虚构 GGUF 路径，在实际读模型之前精确拒绝 `requires identical ordered five RX 6800 target/draft ranks, tensor split, and RELAY/F32`；不是网络依赖的 `test-arg-parser` 全套通过声明。同一新二进制上，默认单卡 Q4_K_M draft／五卡 target 独占、无 profiler 的完整 `1..60` 请求：预热后两次 `predicted_ms=2360.130／2398.842`，逐字正确、自然停止、171/171，Decode **72.45／71.28 tok/s**（`/var/tmp/tp5-mtp-n6-post-native-default-unprofiled.json`）。`--spec-draft-override-tensor '^output.weight$=Vulkan0'` 在五卡 TP5 draft 加载阶段明确拒绝 `pre-allocated tensor (output.weight) in a buffer (Vulkan0) that cannot run the operation (NONE)`；不能通过独立 LM head 缓冲 override 偷换 Meta axis-0 分片的全局 greedy 合并，也没有产生可测性能臂。上述结果保持 >100 验收门未过。
**七行 graph 首次分配的根因实测（仅诊断）**：在 `ggml_backend_sched_alloc_graph`／`ggml_backend_sched_alloc_splits` 临时按 `acquire / split / gallocr first try / backend sync / reserve / gallocr bind` 定界、且两次完整 171/171 正确请求后发现，8336-node 首张七行 graph 的 alloc 约 **104.6–121.7ms**，其中 `ggml_gallocr_alloc_graph` 对 Meta rank-local tensor 的 **binding 99.4–110.1ms**、reserve **5.0–11.6ms**、backend sync **1–2µs**、split **5.8–7.6ms**；不能把这 100ms 笼统归为 GPU kernel 或 driver fence。预热中另一次 8338-node graph bind 135.1ms。把 Meta 的 `simple_tensors` 和 split-state cache 从 `std::map` 换成指针哈希表分别试验后，首次七行 binding 仍为 **94.7–109.5ms**（只省数毫秒，量级未变）；两个容器试验及所有临时 `GGML_TP5_ALLOC_PHASE` 探针已从正式源码撤除，不能当作 >100 收益。若要消灭每请求的首次定义成本，需要保证图身份、五卡 descriptor／scratch、KV／GDN rollback buffer 生命周期的可复用执行程序，而不是只替换容器或复用失效的 UID；即使这约 0.11s 全省，也不能独自补约 0.63s 缺口。
**恢复后的真实验收控制**：撤除上述容器试验／计时探针、重编后的同一二进制五卡 F32/RELAY、Q4_K_M 单卡草稿 n6 独占、无 profiler，`1..60` 预热后两条响应均逐字正确、自然停止、`predicted_n=completion_tokens=171`；`predicted_ms=2372.738／2365.054`，Decode **72.07／72.30 tok/s**，原始 `/var/tmp/tp5-mtp-restored-after-sched-probe-unprofiled.json`。F32 五卡 mesh `--sync relay --rounds 8 --check-all` 包括四轮真实 GPU graph-producer 全绿、FD 27→27；watchdog 保持 active、内核 warning 零条、无残留 server，`git diff --check` 通过。没有新增端到端性能收益，目标仍需 <1710ms。
**拓扑切换和 GPU query 负对照**：五卡 `--split-mode layer`、不启用 TP5 的纯 target 模型实际装载后，`1..5` 正确且自然停止；完整 `1..60` 也逐字正确、171/171、`predicted_ms=9472.466`，只有 **18.05 Decode tok/s**（`/var/tmp/tp5-layer-target-control.json`）。因此不能用层分卡取代当前五卡张量并行目标路由；该臂没有 MTP draft，不与 TP5+MTP 比较净收益。同一 TP5+MTP 171/171 正确请求在 `GGML_VK_PERF_LOGGER=1` 下耗时 **24449ms**，query 禁止命令重放且强烈扰动执行，绝不能作性能基准；它只证明七行图在五卡上仍广泛走 `MUL_MAT_VEC q5_K/q8_0`、`MUL_MAT_ID iq4_nl/iq2_s/iq3_*` 与 `GATED_DELTA_NET` 等多个算子族，不存在单个可由一行 flag 免费消除的 0.63s。现有 `mul_mat_vec_max_cols=18` 明确将 n=7 保持在 direct-quant GEMV 路径；此前 prefill 的 n=19–32 GEMM→GEMV 真机对照与较小 patch 不足以证明反向 GEMV→GEMM 可加速 n=7，未擅改此阈值。
**Meta 热路径卫生**：删除 `ggml_backend_meta_graph_compute` 无条件 `static int meta_compute_calls` 及定期 `fprintf(stderr)`，它在多并发上下文下是未同步递增的数据竞争且不参与图定义／提交。重编后五卡 n6、F32/RELAY、无 profiler 的完整 `1..60` 预热后两次仍逐字正确、自然停止、171/171，`predicted_ms=2342.692／2385.460`（**72.99／71.68 tok/s**；`/var/tmp/tp5-mtp-meta-log-clean-unprofiled.json`），F32 mesh 包括四轮真实 producer 全绿、FD 27→27。此清理没有可认定的性能净收益，不改变 >100 判定。
**五 rank draft 分片 GPU 贪心已证可行、但两次性能门均失败并撤除**：临时在 Meta 把 axis-0 单行 `GGML_OP_ARGMAX` 降为五 rank 本地 GPU ArgMax，回读 `(局部 index, 对应原始 F32 logit)` 后按全局 vocab offset 与最小 index tie-break 合并；仅 MTP、Meta 输出、一枚 greedy sampler 放行，其他 sampler 继续 CPU fallback。五块真实 RX6800 的不等长词表模型外测试覆盖唯一最大、跨 rank／rank 内平局，CPU Meta 边界测试通过。真机发现 MTP catch-up 的 `result_output` 可为 `ne[1]=0`：原 `build_sampling` 仍造 `PAD`，使 Meta 在零元素输入上精确 UNKNOWN 拒绝；现仅当 `t_logits->ne[1]>0` 才建采样图，保留原始 fail-closed 断言。修后 `1..5`、三次完整 `1..60` 均正确、自然停止、171/171。串行逐 rank 两次 D2H 的无 profiler正式两条 **52.99／52.93 tok/s**（`/var/tmp/tp5-five-rank-gpu-global-greedy.json`）；按阶段并发提交五张 index D2H、同步后并发提交五张 logit D2H 的两条 **53.93／53.38 tok/s**（`/var/tmp/tp5-five-rank-gpu-greedy-parallel-readback.json`）。均慢于同构五卡 host-hidden CPU fallback **56.70／56.80**，且远低于单卡草稿 ~72；因此已删除 ArgMax 新降层、readback helper、模型启用门和其专属测试，仅保留无输出时不构建非法采样图的正确性修复。若未来实现，每 rank 在一次 GPU kernel 同时发布 float 最大值和全局 id 并合并，才有避免第二次串行设备读回的成本结构；即便此路线零成本消失，也不能单独证明 <1710ms。
**撤销后的正式验证**：重编后默认五卡 F32/RELAY target、单卡 Q4 draft n6，无 profiler 独占模型预热＋两次完整 `1..60` 均逐字正确、自然停止、171/171，`predicted_ms=2370.901／2376.041`（**72.12／71.97 Decode tok/s**；`/var/tmp/tp5-restored-after-greedy-rejection-unprofiled.json`）。同 server 流式请求读四个 chunk 后断连，日志确实记 `cancel task`，随后的 171/171 仍正确（`/var/tmp/tp5-restored-after-greedy-cancel-recovery.json`）。另起五 rank draft 同样成功 171/171，日志明确 CPU fallback（`/var/tmp/tp5-restored-five-rank-cpu-fallback.json`）；不能把这一次预热 `predicted_ms=3349.166` 与前次正式复跑组成配对差分。`test-meta-reduce-boundary`、`test-mtp-workspace`、`test-predefined-capacity`、F32 五卡 RELAY mesh 含四次真实 producer 全过且 FD 27→27，GPU watchdog active、内核 warning 空、无残留 server，`git diff --check` 通过。上述验证不满足 171 token <1710ms，目标仍开放。
额外 n6 加载后／请求后 Vulkan0 GTT **19.4／25.5MB**，n10 **978.3／985.2MB**，其它 n10 目标卡 GTT 约 **119.5MB**；该差异在请求前已经存在。n10 `GGML_VK_MEMORY_LOGGER=1` 显示 Vulkan0 逻辑 device-local 缓冲 **16.08GiB** 而显式 host buffer 仅约 **4.14MiB**，记录在 `/var/tmp/tp5-mtp-n10-vulkan0-allocator.log`。因此不可将 978MB GTT 直接命名为显式 host buffer 或特定 draft Q4_K 权重；要分辨驱动迁移是否导致同形 Q4_K 核慢，需要缓冲身份／实际驻留证据。另一次只对装载做的 n10 `-c 256→208` 试验中，Vulkan0 GTT **仍是 978264064 bytes** 且 VRAM **仍是 16337448960 bytes**；没有证明缩小 context 的模型性能或完整 171-token 兼容性。
同一逻辑 Vulkan0 分配账中，五 rank 相同的两块 target compute scratch 从 n6 的 **787.99／34.14MiB** 增至 n10 **999.54／238.73MiB**（每卡约 +416MiB）；其后才装载 draft 的三块大缓冲 **545.17／850／914.45MiB**，大小在 n6／n10 **不变**。临时 `GGML_TP5_ALLOC_PROBE=1` 的 n10 预留图将 27 枚 GDN 输出逐一标为 `GATED_DELTA_NET`、每枚 `ne=6144×1440`、**35,389,440 bytes**，原始 `/var/tmp/tp5-mtp-n10-graph-large-tensors.log`。`src/models/delta-net-base.cpp::build_recurrent_attn` 明确把 `K` 个完整 F32 state snapshot 从输出写入 recurrent rollback bank，故不能为了减 scratch 直接丢弃这些快照；只有保持每个可接受前缀的精确状态且通过取消／回滚门，才能改变此内存契约。探针已删除，原源码重编且五卡 F32 RELAY mesh 8 轮（含四轮真实图生产者）全绿；以上定位不等于证明该 416MiB 全由 GDN 构成或改写可节约所需 0.61s。

**本轮退选后的原路径复验**：临时两设备草稿开关、词表头挪卡和顺序 BAR 拷贝均从源码撤销，重建后独占五卡、无 profiler 的原始 Q4_K_M 单卡 n=6 预热及两次重复均逐字准确自然停止、171/171；正式 Decode **72.23／72.60 tok/s**（`predicted_ms=2367.317／2355.337`），`/var/tmp/tp5-mtp-n6-restored-post-trials.json`。F32 RELAY mesh `--rounds 8 --check-all` 全绿、FD 增量零；watchdog enabled/active、近 20 分钟内核 warning 零条、无残留 server，`git diff --check` 干净。上述顺序 BAR 拷贝试验是本节后文已有 `q8-meta ... staged` 退选路径的**重复复核**，不应计为新的独立优化。当前仍需至少约 0.63s Decode 缩短，**未达到 >100**。

**完整 48 段账揭示前三层一致性慢点，尚无修复（诊断，不作性能验收）**：无额外源码修改、只开现有 `GGML_TP5_GPU_TIMING=30/50` 的两条不同 48-stage RELAY replay chain，rank0 第 0／1／2 段 compute 分别约 **4.66／3.75／3.04ms** 与 **4.42／3.76／3.02ms**；另四张卡的前三段和约 **9.88–10.18ms**，同一链后续同类 recurrent 段中位数各约 **0.77–0.80ms**。第 30 条链全五 rank 原始逐段数据 `/var/tmp/tp5-mtp-n6-full48-stages-profile.log`，汇总 `/var/tmp/tp5-mtp-n6-first3-stage-summary.json`。为了辨别节点种类，另一次仅 rank0 的 stage0／4 各 88 个 dispatch 插入成对 GPU 时间戳并给现有 replay tape 加临时 pipeline label：第 0 段 Q5_K 三次合计 **1.147ms**、Q8_0 六次 **0.905ms**、GDN 单核 **0.167ms**，第 4 段分别 **0.256ms**、七次 **0.161ms**、**0.040ms**；两段 **只有 6/88 个索引同名**，不能按 dispatch 索引相减、更不能将这些打点时间当真实 Decode 收益。第一段也多 Q6_K 两次和输入路径，不能把 4–5 倍现象未经控制定性为显存迁移或 DPM 降频；只读 `freq1_input` 采样记录可见动态频率但与 GPU query 无同一时基的精确关联。源算子输出 `/var/tmp/tp5-mtp-n6-stage0-vs4-pipeline-profile.json`、完整日志 `/var/tmp/tp5-mtp-replay-labeled-early-stage0-stage4-ops.log`。临时 query/pipeline-name 改动均撤销，恢复构建待最终未开 profiler 全回答复验；即使假设每次 25 个 target 图都能消除约 **9ms** 的前三段差异，最多也只是约 **225ms** 的推算，距离 <1710ms 尚有约 420ms；**既无证明该差异可消除，也未达到 >100**。

**上述早期阶段诊断现已回退验收闭合**：正式代码中的临时 `early_op` query pool、pipeline-name 捕获、时钟采样进程全部撤除，重编原 `llama-server` 和五卡 mesh；F32 RELAY mesh 8 轮＋四轮真实 GPU graph-producer 全绿且 FD 增量零。独占五卡、未开任何 profile 的 n=6 原始 Q4_K_M 草稿同进程预热＋两次 `1..60` 全部 HTTP 200、逐字正确、自然停止、171/171；正式 `predicted_ms=2414.208／2337.565`，即 **70.83／73.15 Decode tok/s**，`/var/tmp/tp5-mtp-final-restored-unprofiled.json`。watchdog 仍 enabled/active，最近 15 分钟无内核 warning，server 无残留，临时调试符号 grep 零残留、`git diff --check` 通过。**没有端到端净收益；100 tok/s 目标仍未达成**。

**早期时钟诊断的 PCI 索引校准（2026-09-24）**：当前 `vulkaninfo --summary` 的物理设备 UUID 及后端 `enumeratePhysicalDevices()` 顺序一致：Vulkan0..4 此时分别是 BDF **03:00、06:00、09:00、0c:00、0f:00**；之前几个独立进程的物理 VRAM 样本并未同步保存该映射，**不能回溯断言它们也按此顺序枚举**，草稿驻留推断仍未证实。一次临时、只读 `freq1_input` 采样在 RELAY 每段交接后读取 **09:00=此时 Vulkan2** 和 **03:00=此时 Vulkan0（草稿卡）**，两次完整准确 171-token 响应共形成 52 条 12 段序列：Vulkan2 的第 0／11 段频率中位数 **554.5／1390.5MHz**，Vulkan0 则是 **1683／2213MHz**。原始 `/var/tmp/tp5-mtp-replay-early-stage-clocks.log`；这提示草稿未使用的卡有逐段升频，但**不能解释 rank0 同期前三段约 4–5 倍 compute 差**，因为 rank0 所在 Vulkan0 在第 0 段已达约 1.68GHz、阶段算子组合又不同；交接后的 host 频率样本也不是 GPU kernel 时间戳。采样代码已删除并重编，不写硬件频率、不将带探针的请求用于吞吐验收。
最终恢复源码的独占、无 profiler 控制：原 Q4_K_M、n=6、F32/RELAY、五卡，完整 `1..60` 预热后两次自然停止、逐字正确、`predicted_n=completion_tokens=171`；`predicted_ms=2339.845／2402.616`，Decode **73.08／71.17 tok/s**（预热 **64.02**），`/var/tmp/tp5-mtp-post-clock-probe-unprofiled.json`。`test-vulkan-tp5-mesh --sync relay --rounds 8 --check-all` 含四轮真实 GPU graph-producer 全过、FD 增量 0；watchdog enabled/active、最近十分钟 kernel warning 零、server 无残留、源码无 `GGML_TP5_CLOCK_PROBE`／`tp5-relay-clock`，`git diff --check` 通过。**>100 目标未达**。

**七行 target 的第一次图定义是实测税，但不能单独跨 100**：曾临时扩展 `src/llama-context.cpp` 的 opt-in prefill 图主机账以记录 `LLM_GRAPH_TYPE_DEFAULT` 多行 `[tp5-graph-host]`；该逐图计时探针已撤除，以下只作历史诊断。五卡 n=6 完整准确自然停止的第二次请求中，25 次 target 七行图的首轮 `build=1.959ms、sched_alloc=106.704ms、graph_compute=158.543ms、span=267.356ms`；后 23 次图均复用（`build=alloc=0`），平均 `graph_compute=58.902ms、span=58.929ms`，对应 target 周期均值 `66.19ms`。该计时起于 `mctx->apply()` **之后**，不包含外层验收／采样／状态清理；且 opt-in 账和日志扰动不能当正式未开 profile 吞吐。原始逐图和两次完整响应 `/var/tmp/tp5-mtp-n6-target-graph-host-phases.json`，日志 `MtpN6GraphHostLedger`。若能在不牺牲跨请求资源代际／取消恢复的前提下将首轮额外约 208ms **全消**，未开 profiler 的当前最佳 2.317s Decode 仍约 2.109s、远高于 <1.710s；不能据此盲改 scheduler 缓冲寿命或声称已过目标。须同时降低七行稳态的 GPU／RELAY 图执行时间。
上述计时改动重编后的未开 profiler 独占控制，同进程预热＋两次完整 171-token 逐字正确自然停止仍为 **72.49／73.10 Decode tok/s**（正式 `predicted_ms=2358.934／2339.107`），`/var/tmp/tp5-mtp-n6-final-unprofiled.json`；这是该构建的实际回归验证，不是 >100 或已优化计算的证据。`test-mtp-workspace`、`test-target-capacity` 均通过。

**实际重放的两段算子账（仅诊断，不晋级吞吐）**：一次性在已有 `GGML_TP5_GPU_TIMING=30` 的第 30 条 48-stage chain 内，给 rank0 的 stage 10/11 model tape dispatch 前后插入成对 Vulkan compute timestamp；**保留原有整链 RELAY replay**，而不是使用会改变 replay 的 `GGML_VK_PERF_LOGGER`。五卡同进程三次完整 `1..60` 均逐字正确、自然停止、171/171，额外 op query 与标签源码随后全部撤销。此条注入链 rank0 span **56.200ms**（compute **47.491ms**、P2 **7.797ms**）；stage10 的 88 个 dispatch 测得总 **726.68µs**，其中 Q5_K **242.00**、Q8_0 **154.24**、MoE 专家 **107.00**；stage11 的 76 个 dispatch 总 **657.04µs**，三者分别 **200.12／153.76／109.48µs**，其余含 FA/GDN、非线性和数据搬运。精确管线名／逐 dispatch 原始时间 `/var/tmp/tp5-mtp-replay-on-target-op-stages10-11.json`，完整响应 `/var/tmp/tp5-mtp-replay-op-stage10-pilot.json`。时间戳本身会扰动这两段、其余 46 段尚未逐 op 取样；不能把两段线性外推成 48 段的严格上界、也不能把诊断时 `predicted_ms` 当未开 profiler 的性能。它只证明 Q5、Q8、专家和 P2 **共同**构成真实稳态关键成本，孤立修改一个小 kernel 没有足够的已测预算保证 <1.710s；正式验收仍要五卡未开诊断的完整 171-token 请求。撤销诊断并重编后的独占无 profiler 控制：预热和后两次仍全部逐字准确、自然停止、171/171；后两次 `predicted_ms=2480.006／2338.668`（**68.95／73.12 Decode tok/s**），`/var/tmp/tp5-mtp-post-replay-op-probe-unprofiled.json`。watchdog active、无 GPU kernel warning、无残留 server；**未达到 >100**，本轮没有以测量代码冒充优化交付。

**Q5_K F32 n=6 工作组分列无可复核净收益**：RADV native 7-column 路线 256 VGPR／0 spill／每 SIMD 4 subgroup，尝试只对未融合、非 TP5 wire 输出的 Q5_K×F32 7+ 列将每组拆成 4+3，原生 CPU↔Vulkan0 对拍 7／11 列尾组均过，五卡所有 171-token 回答逐字正确自然停止；但同二进制 2-block exploratory ABBA **51.35→51.70 committed tok/s**，差值受运行波动主导，不能证明增益，且拆组重复读取权重。选项、额外 dispatch、测试夹具现均撤销，不保留未晋级实验开关。代码生成证据 `/var/tmp/tp5-mtp-n6-q5-pipeline-stats_logs/`、客户端试验 `/var/tmp/tp5-mtp-n6-q5-tile-abba2.json`，均不能当 >100 证明。

**Q5_K 七列循环不展开退选（Decode 口径）**：将原生 Q5_K×F32 shader 的激活列循环 `[[unroll]]` 临时改为 `[[dont_unroll]]`，Vulkan0 对 CPU 的 q5_K 矩阵 oracle **11/11**（含七列），明确关闭 MMVQ 的七列代码生成 VGPR **256→80**、无 spill，但真实独占五卡 n=6 单卡草稿、参考数学 `1..60` 的 warmup＋两次完整准确自然停止 171-token Decode 仅 **59.22／66.70／67.09 tok/s**；与改动前无 profiler 的 n=6 **72.75／72.83** 顺序样本相比退速，不能把寄存器下降当作收益。证据 `/var/tmp/tp5-mtp-n6-q5-loop-pilot.json`；已还原原 `[[unroll]]` 并重建，未对该臂做配对或声称 100 tok/s。

**Q5_K 单行工作组同样退选（Decode 口径）**：只对 RDNA2 原生 Q5_K×F32 设单行而非两行工作组，仍保留七列及逐行 F32 计算次序。关闭 MMVQ 时 Vulkan0↔CPU 七列矩阵 oracle **1/1**，RADV 七列 VGPR **256→64**、每 SIMD subgroup **4→16**、均无 spill；但独占五卡、未开 profiler、n=6 的预热＋三次逐字正确、自然停止 171-token Decode 仅 **61.08／64.79／69.53／69.55 tok/s**，三个正式样本的 `predicted_ms=2639.248／2459.435／2458.657`，均慢于先前约 73–74 tok/s 的顺序基线，更不接近 <1710ms。证据 `/var/tmp/tp5-mtp-n6-q5row1-pilot.json`；该 opt-in 已删除并重编。工作组数翻倍下更低寄存器与更高理论占用率均不构成端到端提速证明；非配对比较不用于判断细微差异。

反向尝试四行工作组复用同批激活、减半组数：同样仅作用于 RDNA2 原生 Q5_K×F32，Vulkan0↔CPU 七列矩阵 oracle **1/1**，七列 RADV 仍为 **256 VGPR／0 spill**，但代码 **14840→25500 bytes**。独占五卡无 profiler 的 n=6 预热＋三次精确自然停止 171-token Decode **61.55／74.59／73.79／72.84 tok/s**，正式请求 `predicted_ms=2292.462／2317.352／2347.718`，无配对证据证明优于旧二行约 73–74 tok/s，更未达到 100。证据 `/var/tmp/tp5-mtp-n6-q5row4-pilot.json`；四行 opt-in 已删除并重编。原二行二进制再做同进程预热＋两次完整准确自然停止：**73.19／69.64 Decode tok/s**（`predicted_ms=2336.246／2455.572`），后一次明显波动，`/var/tmp/tp5-mtp-n6-post-q5-row-revert.json`，不是四行收益证据。验后载荷 ALU 五卡 **18.084／16.539／18.427／17.362／17.358 TFLOPS**，watchdog enabled/active、无新增内核 GPU fault。1／2／4 行对照表明这里既不能单靠占用率也不能单靠组数提升整张验证热图，继续改这个几何需新的 GPU 算子层计时证据。

**Q8_0 窄输出长 K 已证单算子加速，未证整轮增益**：针对七行图中 `MUL_MAT_VEC q8_0 m=4 n=7 k=10240` 的低并行度，仅在 RDNA2／F32 激活／4 输出／长 K 条件下临时选择 Vulkan 既有 large-WG shader，不另量化激活。Vulkan0↔CPU 同形状 oracle **1/1**，RADV large-WG 为 **48 VGPR／0 spill、LDS 1024B**；同二进制 backend-ops perf 单算子 **61.63→40.83 µs**。跨 subgroup 求和顺序改变，不能宣称 logits 位级等价。独占五卡无 profiler 的 n=6 预热＋三次逐字正确自然停止 171-token Decode **65.93／75.21／72.73／70.11 tok/s**，正式 `predicted_ms=2273.652／2351.075／2439.190`；顺序样本波动大，未证明完整请求稳定快于旧二行／Q8 小工作组，更未过 100。完整响应 `/var/tmp/tp5-mtp-n6-q8-skinny-large-pilot.json`；实验开关和临时 perf 条目已撤销，保留 `test-backend-ops` 的 m4／n7／k10240 CPU 对拍形状。下一步不能从单算子 −20.8 µs 直接外推整轮 −0.61s。

**Q5_K 128-lane K 分摊大退速**：与上述仅 4 输出的 Q8 不同，五卡七行 Q5_K 投影已有数千输出 WG；临时在 RDNA2／F32／m≥2560／k≥2560／≥2 列选择既有 4-subgroup large-WG（128 lane），把每组 K 块分给更多 lane，仍是原 Q5 解码与 F32 激活。模型形状 m2560／n7／k6144 对 CPU oracle **1/1**，RADV **256 VGPR／0 spill、LDS 1024B**；但独占五卡、无 profiler 的 n=6 预热＋三次正确自然停止 171-token Decode 仅 **47.75／54.37／53.21／54.43 tok/s**，正式 `predicted_ms=3145.237／3213.874／3141.623`，比同任务原小 WG 的 69–74 明显慢，更离 100 更远。完整数据 `/var/tmp/tp5-mtp-n6-q5-largewg-pilot.json`；large-WG opt-in 已撤销并重编，保留模型形状 CPU↔Vulkan 回归。原小 WG 二进制恢复后同进程预热＋两次精确自然停止 Decode **73.99／69.15 tok/s**，`predicted_ms=2311.052／2472.822`，证据 `/var/tmp/tp5-mtp-n6-post-q5-large-revert.json`；后一次波动不改变 128-lane 退速结论。跨 subgroup 归并也会重结合 F32，不能仅凭 oracle 通过声称逐位等价。

**七行热图 GPU/主机边界与三项再退选（Decode 口径）**：原始参考 shader 的一次 48-stage RELAY GPU timestamp 观察到 rank0 `span=56.223ms、compute=46.863ms、P2=8.106ms`，是带查询的单链诊断，不是无仪表吞吐；证据 `MtpN6StageTiming` 日志、`/var/tmp/tp5-mtp-n6-original-q5-stage10.json`。另一次 `GGML_TP5_PROFILE_RELAY_STAGES=1` 的正确完整回答中，末次 target 链 48 段 CPU `cpu_data=1.618ms、arm≈0ms、handoff=55.629ms`，说明即使整段 F32 CPU 归约／广播免费也不足以兑现需要的 ~0.61s，不把 GPU P2 的所有等待归罪于 BAR 写入。Q8_0 多行列循环复用反量化权重通过 Vulkan0↔CPU **27/27**，五卡未开 profiler 的单进程四次重复 Decode 为 **74.86／69.94／72.88／74.46 tok/s**；同二进制把 F32 RELAY 改为先写主机 scratch、再连续五次拷贝，另一进程四次重复 **68.78／73.63／72.07／73.56 tok/s**，所有回答精确 171 token、自然停止，但仅顺序探索，无法证明上述任一臂比先前 72–73 更快。证据 `/var/tmp/tp5-mtp-n6-q8-meta-{direct,staged}-pilot.json`，Q8 多行扩展及 staging 开关均撤销。meta tensor cache `std::map`→`unordered_map` 试验把同规模 8336 节点／18160 绑定测成 **89–117ms**，旧 instrumentation 约 **95–123ms**，既未达到预想的 <30ms，也不能从非配对值证明净收益；改动及临时 gallocr 计时钩子均已撤销。回退后独占五卡、无 profiler 的同进程预热＋两次准确自然停止 171-token 请求 Decode **73.02／72.98 tok/s**，`predicted_ms=2341.701／2343.230`，证据 `/var/tmp/tp5-mtp-n6-post-sept24-revert-smoke.json`。验后五卡 1 秒 F32 ALU **18.073／16.581／18.408／17.389／17.373 TFLOPS**，watchdog enabled/active，内核无新增错误。上述路线仍未达到 `predicted_ms<1710` 的验收门。

**七行 HC 融合两级试验亦未晋级**：现有 `ggml_vk_can_fuse_hc_segment` 和 `ggml_vk_can_fuse_hc_fold4` 对普通七行 target 图均拒选；只放开精确 `n_rows=7` 的 opt-in，原生参考 shader 的 whole-region HC 融合使 48-stage rank0 `dispatches=4184→2566、barriers=3293→1771`，同进程三次完整准确自然停止 171-token Decode 却仅 **65.56／68.17／70.43 tok/s**（另有准确短请求 21 token），不能以 dispatch 下降充当速度证据。隔离为仅用既有 `qwen4_hc_up_fold` 合四流而不融合 norm/down/up：`dispatches=3709、barriers=2820`，三次完整准确 171-token Decode **73.94／73.52／73.13 tok/s**，仍离 <1710ms 甚远，且非配对不足以证明相对约 73 tok/s 的小收益。证据 `/var/tmp/tp5-mtp-n6-{hc7,fold7}-trial.json`；两条 opt-in 实验都已撤销，默认数学、active-row 守卫、watchdog 不变。这里验证了输出全文与 token 数，不把它夸称全 logits 逐位等价。

回退重编后独占五卡、无 profiler 的同进程预热＋两次 171-token 准确自然停止 Decode **73.58／74.29 tok/s**（`predicted_ms=2323.930／2301.810`），证据 `/var/tmp/tp5-mtp-n6-post-hc-rollback.json`，仍未达到 100。末次 loaded ALU 五卡 **18.095／16.539／18.433／17.418／17.370 TFLOPS**，watchdog enabled/active、无新内核错误或残留 server。整段融合 dispatch 少了 38.7% 却减速，说明优先瓶颈是额外 fused work 的关键路径而非单纯 dispatch 个数；下一项需先量化单阶段真正可省的计算，而非继续机械放宽 n_rows admission。

**MoE 跨 token 权重复用实测：路由重合是真实的，现有 grouped kernel 却不省完整请求时间**。一次性诊断在五卡纯 Target 的两个正确自然停止 `1..60` 会话中，经每步同步读取 48 层原生 top-10 路由（每会话 170 个单 token 快照，含同进程 warmup），连续 7 步每层 70 次选取平均仅 **35.57／35.58 个不同 expert**；即使每个 WG 只合并两行，理论权重读取组数约 **46.02／70**。诊断带同步及 GPU→CPU 读回，`/var/tmp/tp5-router-overlap-diagnostic.json` 与 `_logs/` **只证明这条计数输出上的路由重合，不能用于吞吐计时，亦非 MTP 验证图本身的路由采样**。据此定向试用现有两行 grouped expert 路由：先 IQ2_S／IQ3_XXS gate/up，2-block 完整正确 ABBA **52.15→51.40**；另为 IQ4_NL down 生成 grouped 变体，初版 Vulkan0↔CPU 对拍抓到第二 reader 激活仍按连续列寻址，修正为实际 packed route 后两组（16 与 512 experts、各 top-10／7 行）均过，2-block ABBA **51.18→51.14**；最后三类一同 grouped 以复用路由准备，四组 CPU 对拍及五卡完整准确回答都通过，2-block ABBA **51.61→50.25 committed tok/s**；全部为同二进制、未开 `GGML_VK_PERF_LOGGER`、各臂预热后 171-token 自然停止的**简并探索**，无正收益或晋级依据；证据 `/var/tmp/tp5-mtp-n6-{grouped-seven,iq4-grouped,all-experts-grouped}-abba2.json`。分桶／indirect dispatch 也增加开销，但当前数据不能将负收益单独归因于它；所有实验性 IQ4 shader、opt-in 分支、诊断钩子及测试夹具均已删除。最终回退构建后 n=6 同进程完整两次 **52.71／52.60 committed tok/s**，171 token／3.244、3.251s，证据 `/var/tmp/tp5-mtp-n6-post-expert-revert-smoke.json`，仍远低于 >100 所需的 <1.71s。

**取消 attention 复制并非本负载的加速臂**：同一构建和显式单设备草稿／CPU sampler／n=6，其余选项相同，B 臂仅设 `GGML_TP5_REPLICATE_ATTN=0`；五卡两臂均按 171 token 逐字正确自然停止。同二进制未开 `GGML_VK_PERF_LOGGER` 的 2-block ABBA **51.39→46.16 committed tok/s**，该较小 exploratory 轮数不提供 CI 或其他负载证明，但明确没有在本任务观察到更快。完整数据 `/var/tmp/tp5-mtp-n6-unreplicated-attn-abba2.json`；开关仅在跑分进程注入，默认复制策略未改。

**恢复验证**：同一生产配置独占 server，先完整 `1..60`（171 token／4.364s），流式 `1..1000` 收到首个 token `1` 后客户端主动断连，再发 `1..10`（21 token／2.140s）与完整 `1..60`（171 token／3.892s）；两条后继均 HTTP 200、逐字准确、自然停止。该次仅验证一次取消链和一组短长混合，不宣称所有时点的异步状态安全；原始响应 `/var/tmp/tp5-mtp-bulk-clear-cancel-recovery.json`，独占 server 已停止。

**复位后的纯 ALU 1 秒实测（本班次）**：逐卡独占 Vulkan FP32 FMA shader，动态循环每线程 8×vec4 FMA（SPIR-V 确认 8 条 vec4 Fma／循环），4096 组×256 lane，每卡 GPU timestamp 累计约 1.01 秒。按执行的 FMA×2 计，card1..5 分别为 **18.027／16.531／18.426／17.331／17.363 TFLOPS**；card2 并非旧时 8.25 TFLOPS 降频态，card5 的真实 SBR 后也没有算力腰斩。证据 `/var/tmp/tp5-alu1s-results.json`，该定向 kernel 与旧 `/tmp/alu_bench` 并非同一实现，**不能将 18.4 对 17.1 的差异当作模型提速**，但足以排除约一半算力的复位降频；测试后无新增内核 GPU fault。

**MTP 的 RADV global BO list 单因素实测（非默认晋级）**：两臂均 `RADV_DEBUG=nobolist`，仅 B 臂在 Vulkan device 创建前加 `GGML_TP5_ISOLATE_BO=1`；相同 server/DSO、五卡身份与 RELAY/F32 单 primary 路由，2-block A/B/B/A 共八个独立进程，每进程预热后做一次完整 `1..60`。A 臂每进程各有五条 `Can't disable the global BO list`，B 臂为零；全部 171 committed tokens、自然停止、文本逐字正确。客户端完整请求速度 A **35.28**、B **35.41 tok/s**（均值，差 +0.13），未见警告所暗示的模型级大幅收益。证据 `/var/tmp/tp5-mtp-bo-isolation-abba2.json`、同名 `_logs/`；两块 `--pilot` **不提供 CI 或默认晋级证明**。禁用 BDA/descriptor indexing/coopmat2 可能使其它 kernel 失去加速，故不自动设此开关；旧 timeline/F16 通信微基准的 77→7 ms 不能外推到当前 RELAY/F32 MTP。试验后每卡再次跑约一秒纯 ALU，card1..5 为 **18.114／16.538／18.424／17.387／17.359 TFLOPS**，没有旧 card2 的算力腰斩；card5 运行中 DPM 读数约 2315–2345 MHz 在 A/B 两臂均出现，不把单个 DPM 星号当成算力证明。watchdog 维持 enabled/active，内核无新 GPU fault。
...
|B08/B09|任务归档按数值／路由／请求墙钟和失败安全分离，否决伪共享与无证据晋级。|旧五块验收在第四块重复请求时 card4 99% busy 并自动恢复；新 F32 MTP 单进程五次完整正确、当前恢复系统已修复旧热摘风险，但跨进程长压测和 >100 committed tok/s 均未验收。**保持显式 opt-in**。|
...
### LateBind HC-down（第二步，2026-09-21，opt-in；以 `TP5-LATEBIND-LINEAR.md` 为准）

`GGML_TP5_LATEBIND=hc-down` 启用 HC-down LateBind；默认关闭。它只在 **RELAY + F32 wire + HC late tensor** 上生效，**不要求 direct producer**，也不改变 `GGML_TP5_REPLICATE_ATTN`。consumer recipe capture 与 producer transport 独立：P1 transport 和显式 direct-host 实验都可进入同一个 LateBind consumer。F16 wire 自动走旧路径，因为 sidecar 若按本地 F32 partial 计算会与 F16-rounded 主 activation 不再严格对应。定义期按 `reference / exact-f32 / aggressive-q8` 三档解析数值模式并单次打印 `[tp5-numerical-mode]`（见 `TP5-LATEBIND-LINEAR.md` 与 `ggml-vulkan-collective.cpp:tp5_resolve_numerical_mode`）；默认走 aggressive Q8（Q8_0 + 整数点积 + F16 sidecar），`GGML_TP5_LATEBIND_EXACT_Q=1`/缺硬件/不对齐/管线未就绪时回 `exact-f32`。

命中 recipe 后，terminal producer 与 P2 之间提前计算四路 scatter 和 `4×rank` 的 W_down sufficient statistic。full-chain 顺序固定为 **producer → 主 P1 publish → late_pre**：CPU 一观察到主 partial ready 就开始五卡 `y` reduce，而同一 GPU queue 继续做 sidecar。Exact 回退：sidecar 先写本地 VRAM，再由单个 64-thread `tp5_hc_publish` 以 128-bit vector 搬到 host-imported bank；Aggressive Q8：各 Q8dot WG 把 F16 sidecar 直接写本地 coherent VRAM 的 `B+128+L`，CPU 在 BAR 映射上原地读并 F16C/AVX2 五路归约，不经 imported system RAM 中转。CPU 有界轮询 sidecar ready 后归约 + fan-out；Y 的 word 0 与 Q 的 word 2 独立发布，不再让已完成的 Y 等 Q。

默认安全路径由 norm/LO 两个消费者在各自首次使用点有界等待：norm（每 stream 一个 256-thread WG，共 4 WG）只等 Y generation，LO（F32 为单个 256-thread WG，Q8 为单个 320-thread WG）只等 Q generation；norm 不写完成 epoch，只有读完两份 payload 的 LO 才写 word 3。这样既不让大 workgroup 承担长自旋，也不把四路 RMS 串在单个 workgroup 上。Exact 回退随后复用既有 W_up/fold；Aggressive 还连带 LO-Q8 量化与 W_up Q8dot（`(width+23)/24` 个 256-thread WG），跳过原 F32 W_up 语义段。`GGML_TP5_LATEBIND_FUSED_FINALIZE=1` 不再是实验入口：它与 Y/Q 独立 generation 不兼容，设置即明确失败（`ggml-vulkan-collective.cpp:7514`）。

LateBind recipe 必须五个 rank 全部匹配（含 `capacity_rows` 一致且 `capacity_rows*streams*rank_dim <= 8192`）；任一 rank 不匹配时整 stage 回旧 RELAY 路径。当前 recipe 要求 4-stream、width 32 对齐、late rank 16 对齐且 ≤512；7 组 push constant 全带 `active_rows`，各 shader 以 `token >= active_rows` 截断，非活跃行永不触碰。`GGML_VK_HC_DOT=q8` / `GGML_VK_HC_DOWN_WG` 不再作为**全局 LateBind 否决条件**：它们仍可作用于其它 HC；命中 LateBind 的这个 W_down 已被 sufficient-statistic 公式替代。对同一个被替代 W_down，activation-Q8 是非线性量化，不能与 exact linear decomposition 同时成立；这是局部数学语义边界，不是允许一个优化全局关闭另一个优化的理由。所有 spin 仍受原固定 `spin_max` 约束，timeout 仍写 sticky failure 并退出。

这里的 exact（`exact-f32`）指**Q 充分统计量不引入量化/近似项**；W_down 被按 rank/stream 重新结合后，F32 加法顺序与旧 kernel 不同，因此不承诺逐位相同。RMS 为每 stream 256-lane tree（width=2560 时每 lane 10 元），scatter 沿用原 Q8 inject 的 FMA/scale 顺序。`aggressive-q8` 则明确引入 Q8_0、F16 sidecar 与重结合差异，必须走中间张量/logits/长解码验收。scatter/rho 按 `capacity_rows=4` 双 bank 复用，不按 plan 分配小块 device memory。Q sidecar 偏移以 `tp5_late_q_*_offset` 为单源契约：fast 路径控制区 `B+64+L`、载荷 `B+128+L`，exact 回退控制 `status[6]`、载荷广播 `B+64+L`。
...
#### 3. multi-row LateBind 的消费者 ABI、生产者捕获与目标容量是三道不同的门
* **历史文档声称**：讨论稿曾将“LateBind”与“旧 fused HC”的行数限制混为一谈，声称 LateBind 仅能支持单行 decode。
* **当前代码事实**：
  - `tp5_late_consumer_ref` 的缓冲尺寸与数值 ABI 允许 $1..4$ 行：`capacity_rows = n_elems / hc.width`，且 `capacity_rows <= VK_TP5_DIRECT_COLUMN_TILE`、`capacity_rows * streams * late_rank <= TP5_LATE_MAX_FLOATS`。此前**只有消费者接受多行**，旧 `ggml_vk_can_fuse_hc_segment` / fold4 与 HC norm/up 着色器只捕获单行，故实机首图 `late=47` 但稳态多行图 `late=0`；不能以消费者门推断整个图已获得收益。
  - 现限定 exact-Q、RELAY/F32 的显式 MTP 实验：生产者为 HC norm、inject、up/fold 按 `[width, 4, n_rows]` 的 token 维寻址，严格限 $1..4$ 行；不满足结构或其它数值模式回退原 reference 图。五卡多行重复请求观察到 `late=46`、完整正确文本及取消恢复，但单次客户端 **34.81** 对 reference **34.98 tok/s**，**无收益晋级**。7 组 LateBind push constants 的行数与 shader 截断契约仍在。
  - Target 主干 GDN/PLE/QSA 的最大容量预定义依旧 fail-closed；以上是每个**精确行数**分别捕获，不能写作 Target 容量化解禁。stage 1 的 PLE 插入也使第一 HC site 不在图首，其余 46 个 site 的捕获与它不同。
