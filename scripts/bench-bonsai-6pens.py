...
def fetch_metrics(base_url, timeout=5.0):
...
def main():
    ...
    m_before_off = fetch_metrics(args.base_url)
    dur_off, resp_off = request_json(endpoint, payload_off, timeout=args.timeout)
    m_after_off = fetch_metrics(args.base_url)
    ...
    dur_on, resp_on = request_json(endpoint, payload_on, timeout=args.timeout)
    m_after_on = fetch_metrics(args.base_url)
