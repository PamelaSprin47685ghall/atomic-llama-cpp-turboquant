...
public:
    ...
    // Bounded-hot state propagates from target attention; legacy defaults
    // would bypass hot reservations entirely.
    ...
    bool rerot_commit_rbb_frontier(
        ...
        return mem_recr ? mem_recr->rerot_commit_rbb_frontier(
            person_id, candidate_seqs, is_public_write, n_candidates) : false;
    }
...
public:
    ...
    // Graph-facing bounded-hot views, forwarded to target attention.
