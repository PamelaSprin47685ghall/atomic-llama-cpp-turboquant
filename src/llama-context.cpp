...
llm_graph_result * llama_context::process_ubatch(const llama_ubatch & ubatch, llm_graph_type gtype, llama_memory_context_i * mctx, ggml_status & ret) {
    ...
    // StateCarryFix instrumented run: post-commit host readback NaN scan of
    // recurrent s_l rows. Safe by construction: the xkv fence above already
    // synchronized the scheduler, and s_l are persistent cache buffers —
    // never scheduler-owned intermediates (cf. the l_out segfault). Scans
    // all rows; the [statecarry] build lines identify the ubatch (ub0=38).
    // NOTE: gated on recurrent layers only, NOT xkv_bounded — the failing
    // config takes stock attention (takes_xkv=0). Fences locally since the
    // xkv fence above only runs when bounded.
    if (mctx) {
        bool sc_any_recr = false;
        for (uint32_t sc_il = 0; sc_il < model.hparams.n_layer(); ++sc_il) sc_any_recr |= model.hparams.is_recr(sc_il);
        if (sc_any_recr) {
            ggml_backend_sched_synchronize(sched.get());
            const auto * sc_hctx = static_cast<const llama_memory_hybrid_context *>(mctx);
            const auto * sc_rctx = sc_hctx ? sc_hctx->get_recr() : nullptr;
            if (sc_rctx) {
                LLAMA_LOG_ERROR("[statecarry] postcommit scan ubatch n_tokens=%d head=%u\n",
                    ubatch.n_tokens, (unsigned) sc_rctx->get_head());
                // StateCarryFix: emulated gather mapping — s_copy values stashed
                // at set_input (same ubatch); marks which s_l rows the next
                // build_rs gather will select.
                {
                    char sc_buf[512]; int sc_pos = 0;
                    sc_pos += snprintf(sc_buf + sc_pos, sizeof(sc_buf) - sc_pos, "%d[", sc_dbg_scopy_ntokens());
                    for (int sc_i = 0, sc_n = sc_dbg_scopy_n(); sc_i < sc_n && sc_pos < (int) sizeof(sc_buf) - 16; ++sc_i)
                        sc_pos += snprintf(sc_buf + sc_pos, sizeof(sc_buf) - sc_pos, "%s%d", sc_i ? "," : "", (int) sc_dbg_scopy(sc_i));
                    snprintf(sc_buf + sc_pos, sizeof(sc_buf) - sc_pos, "]");
                    LLAMA_LOG_ERROR("[statecarry] gather-selects s_copy=%s\n", sc_buf);
                }
                for (uint32_t sc_il = 0; sc_il < model.hparams.n_layer(); ++sc_il) {
                    if (!model.hparams.is_recr(sc_il)) continue;
                    ggml_tensor * sc_st = sc_rctx->get_s_l((int32_t) sc_il);
                    if (!sc_st || !sc_st->buffer || !sc_st->data) continue;
                    if (!ggml_backend_buffer_is_host(sc_st->buffer)) continue;
                    if (sc_st->type != GGML_TYPE_F32) {
                        static bool sc_warned = false;
                        if (!sc_warned) { sc_warned = true; LLAMA_LOG_ERROR("[statecarry] postcommit scan: non-F32 s_l, skipping type check\n"); }
                        continue;
                    }
                    const uint32_t sc_n = (uint32_t) sc_st->ne[0];
                    for (uint32_t sc_r = 0; sc_r < (uint32_t) sc_st->ne[1]; ++sc_r) {
                        const float * sc_row = (const float *) ((const uint8_t *) sc_st->data + (size_t) sc_r * (size_t) sc_st->nb[1]);
                        uint32_t sc_bad = 0, sc_first = 0;
                        for (uint32_t sc_d = 0; sc_d < sc_n; ++sc_d) {
                            if (!std::isfinite(sc_row[sc_d])) { if (sc_bad == 0) sc_first = sc_d; ++sc_bad; }
                        }
                        if (sc_bad > 0) {
                            LLAMA_LOG_ERROR("[statecarry] postcommit NaN: il=%u row=%u: %u/%u elems non-finite (first %u)\n",
                                sc_il, sc_r, sc_bad, sc_n, sc_first);
                        }
                        bool sc_sel = false;
                        for (int sc_k = 0, sc_nsel = sc_dbg_scopy_n(); sc_k < sc_nsel; ++sc_k) {
                            if (sc_dbg_scopy(sc_k) == (int32_t) sc_r) { sc_sel = true; break; }
                        }
                        if (sc_sel) {
                            LLAMA_LOG_ERROR("[statecarry] gather-selects: il=%u row=%u s_l row %s (%u/%u non-finite)\n",
                                sc_il, sc_r, sc_bad ? "NaN" : "finite", sc_bad, sc_n);
                        }
                    }
                }
            }
...
llama_context * llama_init_from_model(
    ...
    // TurboQuant KV cache is not supported for the Inkling architecture: inkling
    // attention runs through the banded flash-attention op
    // (GGML_OP_FLASH_ATTN_EXT_BANDED) with content-dependent relative position
    // bias, which the WHT-rotated turbo cache path does not implement. Fall back
    // to the standard f16 KV cache with a warning instead of failing.
    if (model->arch == LLM_ARCH_INKLING) {
        const auto is_turbo_kv = [](ggml_type t) {
            return t == GGML_TYPE_TURBO2_0 || t == GGML_TYPE_TURBO3_0 || t == GGML_TYPE_TURBO4_0;
        };
        if (is_turbo_kv(params.type_k)) {
            LLAMA_LOG_WARN("%s: TurboQuant K cache type %s is not supported for the inkling arch - falling back to f16\n",
                __func__, ggml_type_name(params.type_k));
            params.type_k = GGML_TYPE_F16;
        }
        if (is_turbo_kv(params.type_v)) {
            LLAMA_LOG_WARN("%s: TurboQuant V cache type %s is not supported for the inkling arch - falling back to f16\n",
