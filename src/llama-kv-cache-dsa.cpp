...
uint32_t llama_kv_cache_dsa::get_kv_capacity() const {
    return kv_mla->get_kv_capacity();
}

uint32_t llama_kv_cache_dsa::get_kv_used() const {
    return kv_mla->get_kv_used();
}

uint32_t llama_kv_cache_dsa::get_kv_seq_used(llama_seq_id seq_id) const {
    return kv_mla->get_kv_seq_used(seq_id);
}
