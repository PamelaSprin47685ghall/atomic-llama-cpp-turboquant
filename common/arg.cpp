...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--tp5-sync"}, "MODE",
        "TP5 sync: relay | timeline | star | l3_star | host | syncfd | gpuflag (default: auto; relay on five RX 6800 ranks)",
        [](common_params & params, const std::string & value) {
            params.tp5.enabled = true;
    ...
    ).set_examples({LLAMA_EXAMPLE_COMPLETION, LLAMA_EXAMPLE_CLI, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_MTMD}).set_env("LLAMA_ARG_CHAT_TEMPLATE_FILE"));
    add_opt(common_arg(
        {"--skip-chat-parsing"},
